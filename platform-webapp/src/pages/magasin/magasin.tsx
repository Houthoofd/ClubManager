...
const MagasinPage: React.FC = () => {
  ...
  useEffect(() => {
    if (articlesParCategorie && panier.length > 0) {
      ...
      panier.forEach((articlePanier, index) => {
        const articleActuel = tousLesArticles.find((a: any) => a.id === articlePanier.id);
        
        if (articleActuel) {
          // Vérifier si les stocks ont changé (comparer les stocks de base, pas ajustés)
          const stocksOriginals = JSON.stringify(articleActuel.stocks?.map((s: any) => ({ taille: s.taille, quantite: s.quantite })));
          const stocksPanier = JSON.stringify(articlePanier.stocks?.map((s: any) => ({ taille: s.taille, quantite: s.quantiteOriginale || s.quantite })));
          
          if (stocksOriginals !== stocksPanier) {
            console.log('🔄 [Panier] Synchronisation article:', articleActuel.nom);
            ...
            dispatch(synchroniserArticle({ index, articleMisAJour: articleSynchronise }));
          }
        }
      });
    }
  ...
  const changerQuantiteArticle = (index: number, quantite: number, taille: string) => {
    console.log('🔄 [Magasin] Changement quantité article:', { index, quantite, taille });
  ...
  const openInfoModal = (article: any) => {
    ...
    const articleActuel = articlesParCategorie ? 
      Object.values(articlesParCategorie)
        .flat()
        .find((a: any) => a.id === article.id) : article;
  ...
  const calculerStocksAjustes = (article: any, panier: Article[]) => {
    ...
    panier
      ...
      .forEach(item => {
        if (item.taille) {
          quantitesReservees[item.taille] = (quantitesReservees[item.taille] || 0) + item.quantite;
          console.log(`🔍 [Panier] Taille ${item.taille}: +${item.quantite} (total: ${quantitesReservees[item.taille]})`);
        }
      });
    ...
    const stocksAjustes = article.stocks.map((stock: any) => {
      ...
      console.log(`🔍 [Stocks] Taille ${stock.taille}: ${stock.quantite} - ${quantiteReservee} = ${nouvelleQuantite}`);
  ...
  useEffect(() => {
    if (isInfoModalOpen && selectedArticle) {
      ...
      const articleActuel = articlesParCategorie ? 
        Object.values(articlesParCategorie)
          .flat()
          .find((a: any) => a.id === selectedArticle.id) : selectedArticle;
      ...
      <PageSection className="store-content">
        ...
        <DetailArticleModal
          ...
          onAjouterAuPanier={(article, taille, quantite) => {
            console.log('🛒 [Modal] Ajout au panier:', { article: article.nom, taille, quantite });
