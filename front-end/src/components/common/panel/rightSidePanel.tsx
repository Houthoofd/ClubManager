...
const RightSidePanel = ({
  ...
  const calculerStocksDisponibles = (
    article: Article,
    articleIndex: number,
  ) => {
  ...
  const getMaxStockForTailleAvecQuantiteTemp = (
    ...
    const stockDisponible = Math.max(
      0,
      stockOriginal.quantite - quantiteReserveeParAutres,
    );
  ...
  const getMaxStockForTailleAvecQuantiteActuelle = (
    ...
    const stockDisponible = Math.max(
      0,
      stockOriginal.quantite - quantiteReserveeTotal,
    );

    console.log(
      `🔢 [RightPanel] Calcul stock AVEC quantité actuelle pour taille ${taille}:`,
      {
        stockOriginal: stockOriginal.quantite,
  ...
  const validerEdition = (index: number) => {
    ...
    const maxStock = getMaxStockForTaille(
      articles[index],
      tailleAUtiliser as string,
      index,
    );
  ...
  const onPasserCommande = async () => {
    ...
    try {
      ...
      console.log(
        "💾 [Panier] SAUVEGARDE CRITIQUE effectuée dans userData et localStorage:",
        {
    ...
    for (let i = 0; i < articlesSnapshot.length; i++) {
      ...
      if (!article.quantite || article.quantite <= 0) {
        console.error(
          `❌ [Panier] Article ${i}: Quantité invalide (${article.quantite})`,
        );
      ...
      if (!article.prix || article.prix <= 0) {
        console.error(
          `❌ [Panier] Article ${i}: Prix invalide (${article.prix})`,
        );
    ...
    console.log(
      "📝 [Panier] Préparation des articles pour la commande (depuis snapshot)...",
    );
    ...
    for (let index = 0; index < articlesSnapshot.length; index++) {
      ...
      if (article.stocks && Array.isArray(article.stocks)) {
        const stockTrouve = article.stocks.find(
          (stock: any) =>
            stock.taille &&
            stock.taille.toLowerCase() === article.taille.toLowerCase(),
        );

        if (stockTrouve && stockTrouve.id) {
          taille_id = Number(stockTrouve.id);
          console.log(
            `✅ [Panier] taille_id depuis stock article: ${taille_id}`,
          );
        }
      }
      ...
      if (!taille_id || isNaN(taille_id) || taille_id <= 0) {
        console.error(
          `❌ [Panier] IMPOSSIBLE de trouver taille_id pour "${article.taille}"`,
        );
        alert(
      ...
      console.log(
        `✅ [Panier] Article ${index} préparé avec taille_id:`,
        articleCommande,
      );

      // ÉTAPE 4: Validation finale
      if (
        isNaN(articleCommande.article_id) ||
        articleCommande.article_id <= 0
      ) {
        console.error(
      ...
      if (isNaN(articleCommande.taille_id) || articleCommande.taille_id <= 0) {
        ...
        alert(
          `Erreur: ID taille invalide pour "${article.nom}" taille "${article.taille}"`,
        );
        return;
      }
      ...
      if (isNaN(articleCommande.prix) || articleCommande.prix <= 0) {
        console.error(
          `❌ [Panier] prix invalide pour article ${index}:`,
          articleCommande.prix,
        );
    ...
    console.log(
      "📋 [Panier] Articles finaux pour commande avec taille_id:",
      articlesCommande,
    );
    ...
    try {
      ...
      console.log(
        "📡 [Panier] Requête finale avec articles sauvegardés:",
        requestBody,
      );
      ...
      if (response.ok) {
        ...
        const commandeId =
          data.commande_id || data.commande?.id || data.commandeId;

        if (!commandeId) {
          console.error(
            "❌ [Panier] ID de commande manquant dans la réponse:",
            {
              response: data,
          ...
          try {
            ...
            const fallbackCommandeId =
              userDataObj.dernierCommandeId ||
              userDataObj.derniereCommande?.commandeId;
            ...
            if (fallbackCommandeId) {
              ...
              console.log(
                "🔄 [Panier] Redirection avec fallback vers:",
                paiementUrl,
              );
            ...
            console.error(
              "❌ [Panier] Erreur fallback userData:",
              fallbackError,
            );
          }

          alert(
            "Erreur: ID de commande non retourné par le serveur. Veuillez réessayer.",
          );
          return;
        }
        ...
        try {
          ...
          console.warn(
            "⚠️ [Panier] Erreur sauvegarde ID commande:",
            idSaveError,
          );
        }
        ...
        if (errorData.error?.includes("Articles manquants")) {
          console.error(
            "❌ [Panier] DEBUG ARTICLES MANQUANTS AVEC SAUVEGARDES:",
          );
          console.error(
            "❌ [Panier] Snapshot articles (sauvegardé):",
            articlesSnapshot,
          );
          console.error("❌ [Panier] Articles props (actuels):", articles);
          console.error(
            "❌ [Panier] Articles Redux (actuels):",
            panierRedux.articles,
          );
          console.error("❌ [Panier] Articles préparés:", articlesCommande);
          console.error(
            "❌ [Panier] Requête envoyée:",
                        ...
                        {editingIndex === index ? (
                          ...
                          <div style={{ padding: "1.25rem" }}>
                                ...
                                <SelectList>
                                  {article.stocks
                                    ...
                                    .map((stock: any, i: number) => {
                                      const stockReellement =
                                        getStockReellementDisponible(
                                          article,
                                          stock.taille,
                                          index,
                                        );
                            ...
                            <div style={{ marginBottom: "1.25rem" }}>
                                ...
                                <TextInput
                                  ...
                                  onChange={(_e, value) => {
                                    ...
                                    const maxStock = selectedTaille
                                      ...
                                      : 0;
                                    setQuantiteTemp(
                                      Math.min(Math.max(1, newValue), maxStock),
                                    );
                                  }}
                                  min="1"
                                  max={
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const maxStock = selectedTaille
                                      ...
                                      : 0;
                                    setQuantiteTemp(
                                      Math.min(quantiteTemp + 1, maxStock),
                                    );
                                  }}
                                  isDisabled={
                                    !selectedTaille ||
                              ...
                              >
                                Sous-total:{" "}
                                {(article.prix * quantiteTemp).toFixed(2)} €
                              </div>
                            </div>
                          ...
                          <div style={{ padding: "1.25rem" }}>
                            ...
                            >
                              ...
                              >
                                {(
                                  article.prix * (article.quantite || 0)
                                ).toFixed(2)}{" "}
                                €
                              </div>
                            </Flex>
