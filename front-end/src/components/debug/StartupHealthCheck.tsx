...
const StartupHealthCheck: React.FC<StartupHealthCheckProps> = ({ onHealthStatus }) => {
      ...
      try {
        ...
        const frontendHealth = {
          stripe_key: !!(import.meta.env.VITE_STRIPE_PUBLIC_KEY && 
                         import.meta.env.VITE_STRIPE_PUBLIC_KEY.length > 50 &&
                         import.meta.env.VITE_STRIPE_PUBLIC_KEY.startsWith('pk_')),
          api_url: !!(import.meta.env.VITE_API_BASE_URL && 
                     import.meta.env.VITE_API_BASE_URL.includes('http')),
          config_loaded: !!(import.meta.env.VITE_STRIPE_PUBLIC_KEY || import.meta.env.VITE_API_BASE_URL)
        };
        ...
        try {
          ...
          const healthResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/health/database`, {
            method: 'GET',
            credentials: 'include'
          });
          ...
          if (backendHealth.reachable) {
            try {
              ...
              if (token) {
                const stripeResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL}/paiements/stripe/diagnostic`, {
                  method: 'GET',
                  headers: {
              ...
              console.warn('⚠️ [StartupHealthCheck] Diagnostic Stripe non disponible:', stripeError);
            }
          }
  ...
  const getStatusIcon = (status: boolean) => {
    return status ? 
      <Label color="green" icon={<CheckCircleIcon />}>OK</Label> :
      <Label color="red" icon={<TimesCircleIcon />}>KO</Label>;
  };

  return (
    <Card>
      <CardTitle>
        🏥 Santé du Système au Démarrage
      </CardTitle>
      <CardBody>
        ...
        <DescriptionList>
            ...
            <DescriptionListDescription>
              <div>
                ...
                <div style={{ fontSize: '12px', marginTop: '2px' }}>
                  {import.meta.env.VITE_STRIPE_PUBLIC_KEY?.substring(0, 25)}...
                </div>
              </div>
          ...
          <DescriptionListGroup>
            ...
            <DescriptionListDescription>
              <div>
                <strong>Accessibilité:</strong> {getStatusIcon(healthStatus.backend?.reachable || false)}
              </div>
              {healthStatus.backend?.stripe_diagnostic && (
                <div style={{ marginTop: '5px' }}>
                  <strong>Stripe Backend:</strong> {getStatusIcon(healthStatus.backend.stripe_diagnostic.overall_health)}
                  <div style={{ fontSize: '12px', marginTop: '2px' }}>
          ...
          {healthStatus.backend?.stripe_diagnostic && (
            <DescriptionListGroup>
              ...
              <DescriptionListDescription>
                <div>
                  <strong>Comptes:</strong> {getStatusIcon(
                    healthStatus.backend.stripe_diagnostic.diagnostic?.compatibility?.account_match || false
                  )}
                </div>
                <div style={{ marginTop: '5px' }}>
                  <strong>Types:</strong> {getStatusIcon(
                    healthStatus.backend.stripe_diagnostic.diagnostic?.compatibility?.both_test_mode ||
                    healthStatus.backend.stripe_diagnostic.diagnostic?.compatibility?.both_live_mode || false
                  )}
                </div>
              </DescriptionListDescription>
            </DescriptionListGroup>
        ...
        {!healthStatus.overall && (
          <Alert variant="warning" title="Actions recommandées" style={{ marginTop: '1rem' }}>
            <ul>
              ...
              {!healthStatus.frontend.api_url && (
                <li>Vérifiez la variable VITE_API_BASE_URL</li>
              )}
              {!healthStatus.backend?.reachable && (
                <li>Vérifiez que le serveur backend est démarré</li>
              )}
              {healthStatus.backend?.stripe_diagnostic && !healthStatus.backend.stripe_diagnostic.overall_health && (
