...
const StatistiquesUtilisateur: React.FC<StatistiquesUtilisateurProps> = ({
  ...
  const totalPresences = statFrequentation.frequentationParMois?.reduce(
    (acc: number, item: any) => acc + item.frequentation, 0
  ) || 0;

  const moyenneTauxPresence = statFrequentation.frequentationParMois?.length > 0
    ? statFrequentation.frequentationParMois.reduce(
        (acc: number, item: any) => acc + item.pourcentage_de_cours_valides, 0
      ) / statFrequentation.frequentationParMois.length
    : 0;

  const meilleurMois = statFrequentation.frequentationParMois?.reduce(
    (best: any, current: any) => 
      current.pourcentage_de_cours_valides > (best?.pourcentage_de_cours_valides || 0) 
        ? current 
        : best,
    null
  );
