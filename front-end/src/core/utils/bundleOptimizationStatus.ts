...
export function displayBundleOptimizationStatus(): void {
  if (!import.meta.env.DEV) return; // Uniquement en développement
  ...
  console.log('%c📊 DÉTAILS DES OPTIMISATIONS:', 'color: #00d4ff; font-weight: bold; font-size: 14px;');
  console.log('\n');
  ...
  if (percentage < 100) {
    ...
    if (!OPTIMIZATIONS[2].implemented) {
      console.log('%c📍 ÉTAPE 1: Installer l\'analyseur de bundle', 'color: #ffd93d; font-weight: bold;');
      console.log('%c   Cela vous permettra de visualiser exactement ce qui prend de la place.', 'color: #a8dadc;');
      console.log('');
      ...
      console.log('   //   visualizer({ filename: \'./dist/stats.html\', open: true, gzipSize: true })');
      console.log('   // ]');
      console.log('');
    ...
    if (!OPTIMIZATIONS[0].implemented) {
      console.log('%c📍 ÉTAPE 2: Implémenter le Code Splitting (PRIORITÉ HAUTE)', 'color: #ffd93d; font-weight: bold;');
      console.log('%c   ⭐ PLUS GROS IMPACT: -40% sur le bundle initial !', 'color: #ff6b6b; font-weight: bold;');
      console.log('');
      console.log('%c   Lazy load des routes dans src/app/routes/index.tsx:', 'color: #00ff88; font-weight: bold;');
      console.log('');
    ...
    if (!OPTIMIZATIONS[3].implemented) {
      console.log('%c📍 ÉTAPE 3: Supprimer dépendances inutilisées', 'color: #ffd93d; font-weight: bold;');
      console.log('');
      console.log('%c   Commandes:', 'color: #00ff88; font-weight: bold;');
    ...
    if (!OPTIMIZATIONS[1].implemented) {
      console.log('%c📍 ÉTAPE 4: Dynamic Imports (composants lourds)', 'color: #ffd93d; font-weight: bold;');
      console.log('');
      console.log('%c   Lazy load Recharts (120KB) uniquement sur page Stats:', 'color: #00ff88; font-weight: bold;');
      console.log('   const BarChart = lazy(() =>');
      console.log('     import(\'recharts\').then(mod => ({ default: mod.BarChart }))');
      console.log('   );');
      console.log('');
      console.log('%c   Lazy load Stripe uniquement au checkout:', 'color: #00ff88; font-weight: bold;');
      console.log('   const StripeForm = lazy(() => import(\'@/features/shop/components/StripeForm\'));');
      console.log('\n');
    }

    if (!OPTIMIZATIONS[4].implemented) {
      console.log('%c📍 ÉTAPE 5: Activer Brotli Compression (optionnel)', 'color: #ffd93d; font-weight: bold;');
      console.log('');
      console.log('%c   Commandes:', 'color: #00ff88; font-weight: bold;');
    ...
    console.log('\n');
    console.log('%c💰 GAINS ATTENDUS (avec toutes optimisations):', 'color: #00d4ff; font-weight: bold; font-size: 14px;');
    console.log('');
    console.log('%c   Initial Bundle:  888KB → ~350KB (gzip)  ⬇️ -60%', 'color: #00ff88; font-size: 13px;');
    console.log('%c   Total Bundle:    ~3MB → ~1.5MB          ⬇️ -50%', 'color: #00ff88; font-size: 13px;');
    console.log('%c   TTI (Time to Interactive): 3.5s → ~1.5s  ⬇️ -55%', 'color: #00ff88; font-size: 13px;');
    console.log('');
  } else {
    console.log('\n');
    console.log('%c🎉 FÉLICITATIONS !', 'color: #00ff88; font-weight: bold; font-size: 16px;');
    console.log('%c   Toutes les optimisations sont implémentées !', 'color: #00ff88; font-size: 13px;');
    console.log('\n');
  }
