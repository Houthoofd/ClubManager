        ...
        <CardBody>
          <div className="messages-header">
            ...
            <p className="messages-subtitle">
              Messages qui nécessitent votre attention ({messagesNonLus.length} message
              {messagesNonLus.length > 1 ? "s" : ""})
            </p>
          </div>
          ...
          {messagesNonLus.length === 0 ? (
            <EmptyState
              icon={InboxIcon}
              title="Tous vos messages sont lus !"
              description="Félicitations, vous n'avez aucun message non lu."
              iconColor="#28a745"
            />
          ) : (
                      ...
                      <div style={getDateStyle(message)}>
                        ...
                        {message.lu && message.date_lecture && (
                          <span style={{ marginLeft: "15px" }}>
                            <strong>Lu le:</strong>{" "}
                            {new Date(message.date_lecture).toLocaleString()}
                          </span>
                        )}
                      </div>
                    </div>
