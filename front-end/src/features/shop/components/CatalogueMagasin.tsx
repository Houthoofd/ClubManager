...
const CatalogueMagasin: React.FC<CatalogueMagasinProps> = ({
  ...
  return (
    <PageSection>
      {Object.entries(articlesParCategorie || {}).map(
        ([categorie, articles], idx) => (
          <div key={`${categorie}-${refreshKey || 0}`}>
            {idx > 0 && (
              <Divider
                style={{ margin: "2rem 0", borderTop: "2px solid #dee2e6" }}
              />
            )}
                      ...
                      <GalleryItem key={`${article.id}-${refreshKey || 0}`}>
                        ...
                        >
                          <ArticleCard
                            ...
                            onAddToCart={(taille: string) =>
                              onAjouterAuPanier(article, taille)
                            }
