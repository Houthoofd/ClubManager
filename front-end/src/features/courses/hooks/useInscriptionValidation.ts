...
export const useInscriptionValidation = (
  form: FormData,
    ...
    (fieldName: string, value: string) => {
      ...
      // Validation avec le schéma Zod pour les autres champs
        ...
        try {
          ...
          const fieldSchema =
            userInscriptionSchema.shape[
              fieldName === "date_naissance"
                ? "date"
                : (fieldName as keyof typeof userInscriptionSchema.shape)
            ];
          ...
          if (fieldSchema) {
            ...
            fieldValidation = {
              ...
              message: value.trim().length > 0 ? "" : "Ce champ est requis",
            };
          }
        } catch (error: any) {
  ...
  const checkFormComplete = useCallback(() => {
    ...
    if (isComplete && allFieldsValid && passwordsMatch) {
      try {
        ...
        if (!fullData.date || !/^\d{4}-\d{2}-\d{2}$/.test(fullData.date)) {
          ...
          console.log(
            "❌ [checkFormComplete] Abonnement invalide:",
            form.abonnement,
            "->",
            fullData.abonnement,
          );
            ...
            if (result.error && result.error.errors && Array.isArray(result.error.errors)) {
              result.error.errors.forEach((err, index) => {
                console.log(`Erreur ${index + 1}:`, {
                  ...
                  valeur_fournie: err.path
                    ? err.path.reduce((obj, key) => obj?.[key], fullData)
                    : "N/A",
                });
              });
            } else {
    ...
    console.log("🔍 [checkFormComplete] Vérification complétude avec Zod:", {
      ...
      validation: Object.keys(validation).reduce(
        (acc, key) => ({
          ...acc,
          [key]: {
