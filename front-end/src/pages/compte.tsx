...
const Compte = () => {
  ...
  useEffect(() => {
    if (compteInfo) {
      console.log(
        "🔍 CompteInfo data COMPLET:",
        JSON.stringify(compteInfo, null, 2),
      );
        ...
        date_naissance:
          formatDateForInput(
            compteInfo.date_naissance ||
              compteInfo.utilisateur?.date_of_birth ||
              "",
          ) || "",
    ...
    switch (key) {
      case "genres":
        const genre = genres?.find(
          (g) => String(g.id) === stringValue || g.genre_name === stringValue,
        );
        return genre?.genre_name || stringValue;
      case "grades":
        const grade = grades?.find(
          (g) => String(g.id) === stringValue || g.grade_id === stringValue,
        );
        return grade?.grade_id || stringValue;
      case "abonnement":
        const abonnement = abonnements?.find(
          (a) => String(a.id) === stringValue || a.nom_plan === stringValue,
        );
        return abonnement?.nom_plan || stringValue;
      case "status":
        const statusItem = status?.find(
          (s) => String(s.id) === stringValue || s.nom_status === stringValue,
        );
  ...
  const handleApplyChanges = async () => {
    ...
    if (Object.keys(changes).length > 0) {
      const modifications: ModificationItem[] = Object.entries(changes).map(
        ([key, newValue]) => {
          let originalValue =
            compteInfo?.utilisateur?.[key] || compteInfo?.[key];
          ...
          if (
            ...
            if (key === "genres" && genres) {
              ...
              let abonnement =
                abonnements.find((a) => a.nom_plan === originalString) ||
                abonnements.find((a) => String(a.id) === originalString);
              originalDisplay =
                abonnement?.nom_plan || `Abonnement: ${originalString}`;
            } else if (key === "status" && status) {
              let statusItem =
                status.find((s) => s.nom_status === originalString) ||
                status.find((s) => String(s.id) === originalString);
              originalDisplay =
                statusItem?.nom_status || `Status: ${originalString}`;
