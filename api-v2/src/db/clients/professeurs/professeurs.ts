...
export class Professeurs {
  ...
  async obtenirPlanningCoursProfesseur(inputId: number): Promise<VerifyResultWithData> {
    const sql = `
      SELECT
    ...
    return new Promise<VerifyResultWithData>((resolve, reject) => {
      this.mysqlConnector.query(sql, [inputId, inputId, inputId], (error, results) => {
        ...
        if (results.length > 0) {
          ...
          const planningCours = results.map((result: any) => ({
  ...
  async obtenirUtilisateurParId(id: number): Promise<any | null> {
