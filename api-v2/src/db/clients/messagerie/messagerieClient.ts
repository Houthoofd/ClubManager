...
export class MessagerieClient {
  ...
  async getAllTypesMessages(): Promise<any[]> {
  ...
  async getTypeMessageById(id: number): Promise<any | null> {
    const results = await this.queryAsync(
      'SELECT * FROM types_messages_personnalises WHERE id = ?',
      [id]
    );
  ...
  async updateTypeMessage(id: number, title: string, content: string): Promise<boolean> {
    const results = await this.queryAsync(
  ...
  async getMessagesRecus(userId: string): Promise<any[]> {
  ...
  async supprimerMessageRecu(messageId: number, userId?: number): Promise<boolean> {
    const results = await this.queryAsync(
  ...
  async getStatistiquesMessages(userId?: number): Promise<any> {
    const stats: any = {};

    // Nombre total de types de messages
    const totalTypes = await this.queryAsync(
      'SELECT COUNT(*) as count FROM types_messages_personnalises',
      []
    );
    stats.totalTypesMessages = Array.isArray(totalTypes) ? totalTypes[0]?.count : 0;

    // Nombre total de messages envoyés
    const totalMessages = await this.queryAsync(
      'SELECT COUNT(*) as count FROM messages_personnalises',
      []
    );
    ...
    if (userId) {
      const userMessages = await this.queryAsync(
        'SELECT COUNT(*) as count FROM messages_personnalises WHERE utilisateur_id = ?',
        [userId]
      );
    ...
    const messagesParJour = await this.queryAsync(
      `SELECT 
  ...
  async nettoyerAnciennesMessages(joursAConserver: number = 30): Promise<number> {
    const results = await this.queryAsync(
      'DELETE FROM messages_personnalises WHERE created_at < DATE_SUB(NOW(), INTERVAL ? DAY)',
      [joursAConserver]
    );
    return results.affectedRows || 0;
  }
...
// Instance singleton
let messagerieClientInstance: MessagerieClient | null = null;

export function getMessagerieClient(): MessagerieClient {
