...
// Créer des dates dynamiques dans le futur pour les cours mockés
  ...
  constructor() {
    this.alertes_utilisateurs = {
      findMany: (args) => {
        ...
        if (args?.where?.utilisateur_id) {
          results = results.filter(
            (a) => a.utilisateur_id === args.where.utilisateur_id,
          );
        }
      ...
      findFirst: (args) => {
        const result = mockAlertesUtilisateurs.find((a) => {
          ...
          if (args?.where?.utilisateur_id && args?.where?.alerte_type_id) {
            return (
              a.utilisateur_id === args.where.utilisateur_id &&
      ...
      create: (args) => {
        ...
        if (args.include?.alertes_types) {
          newAlerte.alertes_types = mockAlertesTypes.find(
            (t) => t.id === args.data.alerte_type_id,
          );
        }
        if (args.include?.utilisateurs) {
          newAlerte.utilisateurs = mockUtilisateurs.find(
            (u) => u.id === args.data.utilisateur_id,
          );
        }
      ...
      update: (args) => {
        const alerte = mockAlertesUtilisateurs.find(
      ...
      count: (args) => {
        ...
        if (args?.where?.alertes_types?.priorite) {
          results = results.filter(
            (a) =>
              a.alertes_types.priorite === args.where.alertes_types.priorite,
          );
        }
    ...
    this.utilisateurs = {
      findMany: (args) => {
        ...
        if (args?.where?.first_name) {
          results = results.filter(
            (u) => u.first_name === args.where.first_name,
          );
        }
        ...
        if (args?.include) {
          results = results.map((u) => {
            const enriched = { ...u };
            if (args.include.genres)
              enriched.genres =
                mockGenres.find((g) => g.id === u.genre_id) || null;
            if (args.include.grades)
              enriched.grades =
                mockGrades.find((g) => g.id === u.grade_id) || null;
            if (args.include.status)
              enriched.status =
                mockStatus.find((s) => s.id === u.status_id) || null;
            if (args.include.plans_tarifaires)
              enriched.plans_tarifaires =
                mockPlansTarifaires.find((p) => p.id === u.abonnement_id) ||
                null;
            return enriched;
          });
        }
      ...
      findFirst: (args) => {
        let result = mockUtilisateurs.find((u) => {
          ...
          if (args?.where?.first_name)
            match = match && u.first_name === args.where.first_name;
          if (args?.where?.last_name)
            match = match && u.last_name === args.where.last_name;
          if (args?.where?.status_id !== undefined)
            match = match && u.status_id === args.where.status_id;
          return match;
        });
        ...
        if (args?.include) {
          ...
          if (args.include.genres)
            enriched.genres =
              mockGenres.find((g) => g.id === result.genre_id) || null;
          if (args.include.grades)
            enriched.grades =
              mockGrades.find((g) => g.id === result.grade_id) || null;
          if (args.include.status)
            enriched.status =
              mockStatus.find((s) => s.id === result.status_id) || null;
          if (args.include.plans_tarifaires)
            enriched.plans_tarifaires =
              mockPlansTarifaires.find((p) => p.id === result.abonnement_id) ||
              null;
          result = enriched;
        }
      ...
      create: (args) => {
        ...
        const emailExists = mockUtilisateurs.some(
          (u) => u.email === args.data.email,
        );
        if (emailExists) {
      ...
      update: (args) => {
        ...
        if (args?.include) {
          ...
          if (args.include.genres)
            enriched.genres =
              mockGenres.find((g) => g.id === updated.genre_id) || null;
          if (args.include.grades)
            enriched.grades =
              mockGrades.find((g) => g.id === updated.grade_id) || null;
          if (args.include.status)
            enriched.status =
              mockStatus.find((s) => s.id === updated.status_id) || null;
          if (args.include.plans_tarifaires)
            enriched.plans_tarifaires =
              mockPlansTarifaires.find((p) => p.id === updated.abonnement_id) ||
              null;
    ...
    this.password_reset_tokens = {
      findFirst: (args) => {
        ...
        if (result && args?.include?.utilisateurs) {
          result = {
            ...result,
            utilisateurs: mockUtilisateurs.find((u) => u.id === result.user_id),
          };
        }
      ...
      deleteMany: (args) => {
        ...
        if (args?.where?.user_id) {
          count = mockPasswordResetTokens.filter(
            (t) => t.user_id === args.where.user_id,
          ).length;
          ...
          count = mockPasswordResetTokens.filter(
            (t) => t.expires_at < args.where.expires_at.lt,
          ).length;
        }
        return Promise.resolve({ count });
      },
    ...
    this.commandes = {
      findMany: (args) => {
        ...
        if (args?.where?.date_commande?.gte) {
          results = results.filter(
            (c) => c.date_commande >= args.where.date_commande.gte,
          );
        }
        if (args?.where?.date_commande?.lte) {
          results = results.filter(
            (c) => c.date_commande <= args.where.date_commande.lte,
          );
        }
              ...
              return searchOr.some((condition) => {
                if (condition.first_name?.contains) {
                  return user.first_name
                    .toLowerCase()
                    .includes(condition.first_name.contains.toLowerCase());
                }
                if (condition.last_name?.contains) {
                  return user.last_name
                    .toLowerCase()
                    .includes(condition.last_name.contains.toLowerCase());
                }
      ...
      findUnique: (args) => {
        const commande = mockCommandes.find(
      ...
      create: (args) => {
        const newCommande = {
          ...
          updated_at: new Date(),
          utilisateurs: mockUtilisateurs.find(
            (u) => u.id === args.data.utilisateur_id,
          ),
        };
      ...
      update: (args) => {
        const index = mockCommandes.findIndex(
          (c) => c.commande_id === args.where.commande_id,
        );
        if (index === -1) {
        ...
        if (args?.include?.cours_recurrent_professeur && result) {
          ...
          result = {
            ...
            cours_recurrent_professeur: associations.map((assoc) => ({
              ...assoc,
              cours_recurrent: mockCoursRecurrent.find(
                (cr) => cr.id === assoc.cours_recurrent_id,
              ),
            })),
          };
        }
    ...
    this.cours_recurrent = {
      findMany: (args) => {
        ...
        if (args?.where?.jour_semaine) {
          results = results.filter(
            (cr) => cr.jour_semaine === args.where.jour_semaine,
          );
        }
        ...
        if (args?.include?.cours_recurrent_professeur) {
          results = results.map((cr) => ({
            ...
            cours_recurrent_professeur: mockCoursRecurrentProfesseur
              ...
              .map((crp) => ({
                ...crp,
                professeurs: mockProfesseurs.find(
                  (p) => p.id === crp.professeur_id,
                ),
              })),
          }));
        }
      ...
      findFirst: (args) => {
        const result = mockCoursRecurrent.find((cr) => {
          if (
            args?.where?.jour_semaine &&
            cr.jour_semaine !== args.where.jour_semaine
          ) {
            return false;
          }
          if (
            args?.where?.type_cours &&
            cr.type_cours !== args.where.type_cours
          ) {
            return false;
          }
          if (
            args?.where?.heure_debut &&
            cr.heure_debut !== args.where.heure_debut
          ) {
            return false;
          }
      ...
      update: (args) => {
        const index = mockCoursRecurrent.findIndex(
          (cr) => cr.id === args.where.id,
        );
        if (index >= 0) {
          mockCoursRecurrent[index] = {
            ...mockCoursRecurrent[index],
            ...args.data,
          };
    ...
    this.cours = {
      findMany: (args) => {
        ...
        if (args?.where?.date_cours?.gte) {
          results = results.filter(
            (c) => c.date_cours >= args.where.date_cours.gte,
          );
        }
        if (args?.where?.date_cours?.lte) {
          results = results.filter(
            (c) => c.date_cours <= args.where.date_cours.lte,
          );
        }
        if (args?.where?.cours_recurrent_id) {
          results = results.filter(
            (c) => c.cours_recurrent_id === args.where.cours_recurrent_id,
          );
        }
      ...
      updateMany: (args) => {
        ...
        mockCours.forEach((c) => {
          let matches = true;
          if (
            args?.where?.cours_recurrent_id &&
            c.cours_recurrent_id !== args.where.cours_recurrent_id
          ) {
            matches = false;
          }
          if (
            args?.where?.date_cours?.gte &&
            c.date_cours < args.where.date_cours.gte
          ) {
            matches = false;
          }
    ...
    this.inscriptions = {
      findMany: (args) => {
        ...
        if (args?.where?.utilisateur_id) {
          results = results.filter(
            (i) => i.utilisateur_id === args.where.utilisateur_id,
          );
        }
      ...
      findFirst: (args) => {
        const result = mockInscriptions.find((i) => {
          if (args?.where?.cours_id && i.cours_id !== args.where.cours_id)
            return false;
          if (
            args?.where?.utilisateur_id &&
            i.utilisateur_id !== args.where.utilisateur_id
          )
            return false;
          return true;
        });
      ...
      create: (args) => {
        ...
        if (args?.include) {
          ...
          if (args.include.cours) {
            result.cours = mockCours.find(
              (c) => c.id === newInscription.cours_id,
            );
          }
          if (args.include.utilisateurs) {
            result.utilisateurs = mockUtilisateurs.find(
              (u) => u.id === newInscription.utilisateur_id,
            );
          }
          return Promise.resolve(result);
        }
      ...
      deleteMany: (args) => {
        ...
        if (args?.where?.cours_id && args?.where?.utilisateur_id) {
          toDelete = mockInscriptions.filter(
            (i) =>
              i.cours_id === args.where.cours_id &&
              i.utilisateur_id === args.where.utilisateur_id,
          );
        } else if (
          args?.where?.cours?.cours_recurrent_id &&
          args?.where?.cours?.date_cours?.gte
        ) {
          const coursIds = mockCours
            .filter(
              (c) =>
                c.cours_recurrent_id === args.where.cours.cours_recurrent_id &&
                c.date_cours >= args.where.cours.date_cours.gte,
            )
            .map((c) => c.id);
          toDelete = mockInscriptions.filter((i) =>
            coursIds.includes(i.cours_id),
          );
        }
