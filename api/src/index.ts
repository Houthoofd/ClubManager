...
if (!fs.existsSync(envPath)) {
  ...
  console.log(
    `⚠️ [Server] Fichier .env.${nodeEnv} non trouvé, utilisation de .env générique`,
  );
}
...
console.log(
  "  FRONTEND_URL_ALT:",
  process.env.FRONTEND_URL_ALT || "NON DÉFINIE",
);
console.log(
  "  FRONTEND_URL_LOCAL:",
  process.env.FRONTEND_URL_LOCAL || "NON DÉFINIE",
);
...
async function startServer() {
  try {
    ...
    while (!dbReady && attempts < maxAttempts) {
      try {
        ...
        if (attempts >= maxAttempts) {
          throw new Error(
            `Base de données non disponible après ${maxAttempts} tentatives`,
          );
        }
    ...
    try {
      ...
      console.warn(
        "⚠️ [Server] Erreur lors de l'initialisation EmailClient:",
        emailError,
      );
      console.warn(
        "⚠️ [Server] Le serveur continuera sans les services email optimaux",
      );
    }
    ...
    try {
      ...
      console.log(
        "📧 [Server] EmailValidationService remplacé par EmailClient - pas d'initialisation requise",
      );
      ...
      console.log(
        "✅ [Server] Services email initialisés (EmailValidationService + EmailClient + MessageClient)",
      );
    } catch (emailError) {
    ...
    try {
      ...
      const diagnosticStripe = async () => {
              ...
              format: process.env.STRIPE_SECRET_KEY
                ? process.env.STRIPE_SECRET_KEY.startsWith("sk_test_")
                  ...
                  : process.env.STRIPE_SECRET_KEY.startsWith("sk_live_")
                    ...
                    : "INVALID"
                : "MISSING",
              account_id: process.env.STRIPE_SECRET_KEY
                ? process.env.STRIPE_SECRET_KEY.substring(8, 23)
                : "N/A",
              ...
              format: process.env.STRIPE_PUBLIC_KEY
                ? process.env.STRIPE_PUBLIC_KEY.startsWith("pk_test_")
                  ...
                  : process.env.STRIPE_PUBLIC_KEY.startsWith("pk_live_")
                    ...
                    : "INVALID"
                : "MISSING",
              account_id: process.env.STRIPE_PUBLIC_KEY
                ? process.env.STRIPE_PUBLIC_KEY.substring(8, 23)
                : "N/A",
            },
          },
          compatibility: {
        ...
        if (process.env.STRIPE_SECRET_KEY && process.env.STRIPE_PUBLIC_KEY) {
          ...
          diagnostic.compatibility.account_match =
            secretAccount === publicAccount;
          diagnostic.compatibility.keys_from_same_account =
            secretAccount === publicAccount;
      ...
      const isFullyOperational =
        ...
        (stripeDiagnostic.compatibility.both_test_mode ||
          stripeDiagnostic.compatibility.both_live_mode) &&
        stripeDiagnostic.stripe_api_test.success;

      console.log("💳 [Server] === DIAGNOSTIC STRIPE DÉMARRAGE ===");
      console.log(
        `💳 [Server] Status global: ${isFullyOperational ? "✅ OPÉRATIONNEL" : "❌ PROBLÈME DÉTECTÉ"}`,
      );
      console.log(
        `💳 [Server] Clé secrète: ${stripeDiagnostic.backend.secret_key.exists ? "✅" : "❌"} ${stripeDiagnostic.backend.secret_key.format} (${stripeDiagnostic.backend.secret_key.account_id})`,
      );
      console.log(
        `💳 [Server] Clé publique: ${stripeDiagnostic.backend.public_key.exists ? "✅" : "❌"} ${stripeDiagnostic.backend.public_key.format} (${stripeDiagnostic.backend.public_key.account_id})`,
      );
      console.log(
        `💳 [Server] Compatibilité: ${stripeDiagnostic.compatibility.account_match ? "✅ Comptes identiques" : "❌ Comptes différents"}`,
      );
      console.log(
        `💳 [Server] API Stripe: ${stripeDiagnostic.stripe_api_test.success ? "✅ Accessible" : "❌ Inaccessible"}`,
      );

      if (stripeDiagnostic.backend.public_key.exists) {
        const expectedFrontendKey = process.env.STRIPE_SECRET_KEY
          ? `pk_${process.env.STRIPE_SECRET_KEY.substring(3)}`
          : "N/A";
        console.log(
          `💳 [Server] Clé frontend attendue: ${expectedFrontendKey.substring(0, 25)}...`,
        );
        console.log(
          `💳 [Server] Clé backend publique: ${stripeDiagnostic.backend.public_key.value.substring(0, 25)}...`,
        );
        console.log(
          `💳 [Server] Correspondance: ${stripeDiagnostic.backend.public_key.value === expectedFrontendKey ? "✅ OUI" : "❌ NON"}`,
        );
      }
      ...
      if (!isFullyOperational) {
        ...
        if (!stripeDiagnostic.backend.stripe_initialized) {
          console.log(
            "💳 [Server]   - Vérifiez la variable STRIPE_SECRET_KEY dans .env",
          );
        }
        if (!stripeDiagnostic.backend.secret_key.exists) {
          console.log(
            "💳 [Server]   - Ajoutez STRIPE_SECRET_KEY dans votre fichier .env",
          );
        }
        if (!stripeDiagnostic.compatibility.account_match) {
          console.log(
            "💳 [Server]   - Assurez-vous que les clés publique/secrète proviennent du même compte Stripe",
          );
        }
        if (
          !stripeDiagnostic.stripe_api_test.success &&
          stripeDiagnostic.stripe_api_test.attempted
        ) {
          console.log(
            `💳 [Server]   - Problème API Stripe: ${stripeDiagnostic.stripe_api_test.error}`,
          );
        }
      }
      ...
      if (!isFullyOperational && process.env.NODE_ENV === "production") {
        console.error(
          "❌ [Server] ERREUR CRITIQUE: Stripe non opérationnel en production !",
        );
        console.error(
          "❌ [Server] Le serveur ne peut pas démarrer sans une configuration Stripe valide.",
        );
        console.error(
          "❌ [Server] Veuillez corriger la configuration et redémarrer.",
        );
        process.exit(1);
      } else if (!isFullyOperational) {
        console.warn(
          "⚠️ [Server] Stripe non opérationnel en développement - le serveur continue mais les paiements ne fonctionneront pas.",
        );
      }
    } catch (diagnosticError) {
      console.error(
        "❌ [Server] Erreur lors du diagnostic Stripe:",
        diagnosticError,
      );
    ...
    const corsOptions = {
      origin: (
        ...
        if (!origin) {
          console.log(
            "🔓 [CORS] Requête sans origin autorisée (Postman, mobile, etc.)",
          );
        ...
        if (allowedOrigins.includes(origin)) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.warn(
              "⚠️ [CORS] Mode développement - autorisation temporaire de:",
              origin,
            );
            callback(null, true);
          } else {
    ...
    const { default: utilisateursRouter } =
    ...
    try {
      ...
      console.error(
        "❌ [Server] Erreur critique lors du chargement du module paiements:",
        error,
      );
    ...
    const { default: statistiquesRouter } =
      await import("./routes/statistiques.js");
    ...
    console.log(
      "ℹ️ [Server] Modules échéances et webhooks intégrés dans le module paiements unifié",
    );
    ...
    if (paiementRouter !== null) {
      ...
      console.error(
        "❌ [Server] CRITIQUE: Module paiements non disponible - création fallback",
      );
    ...
    if (fs.existsSync(frontendPath)) {
      ...
      console.log(
        "📂 [Server] Frontend servi depuis nginx:",
        nginxFrontendPath,
      );
    }
    ...
    if (staticPath) {
      ...
      console.log(
        "✅ [Server] Frontend React configuré avec SPA routing et paiements modulaires",
      );
    } else {
      ...
      app.get("*", (req, res) => {
        ...
        if (apiRoutes.some((route) => req.path.startsWith(route))) {
          return res.status(404).json({
            ...
            debug: {
              ...
              paymentsSubmodule: req.path.startsWith("/paiements/stripe/")
                ? "stripe"
                : req.path.startsWith("/paiements/echeances/")
                  ? "echeances"
                  : req.path.startsWith("/paiements/confirmation/")
                    ? "confirmation"
                    : req.path.startsWith("/paiements/webhooks/")
                      ? "webhooks"
                      : "main",
              timestamp: new Date().toISOString(),
            },
        ...
        res.status(404).json({
          ...
          suggestion:
            "Utilisez les routes API directement ou vérifiez que le frontend est déployé",
        });
      });
    }
        ...
        if (!emailConfigured) {
          return res.status(500).json({
            ...
            email: {
              configured: false,
              error:
                "Variables SENDGRID_API_KEY et/ou SENDGRID_FROM_EMAIL manquantes",
            },
          });
        }
        ...
        try {
          ...
          res.json({
            ...
            email: {
              configured: emailConfigured,
              warning:
                "Service email non initialisé mais configuration présente",
            },
          });
        }
      } catch (error) {
    ...
    server.listen(PORT, () => {
      ...
      if (process.env.STRIPE_SECRET_KEY) {
        const stripeType = process.env.STRIPE_SECRET_KEY.startsWith("sk_test_")
          ? "TEST"
          : "LIVE";
        ...
        console.log(
          `💳 [Server] Diagnostic: http://localhost:${PORT}/paiements/stripe/diagnostic`,
        );
        console.log(
          `💳 [Server] Test compatibilité: POST http://localhost:${PORT}/paiements/stripe/test-compatibility`,
        );
      } else {
      ...
      if (paiementRouter !== null) {
        console.log(
          `💳 [Server] Module paiements: http://localhost:${PORT}/paiements/`,
        );
      ...
      console.log(
        `📊 [Server] Environnement : ${process.env.NODE_ENV || "development"}`,
      );
      console.log(
        `📧 [Server] SendGrid: ${process.env.SENDGRID_API_KEY ? "Configuré" : "Non configuré"}`,
      );
      console.log(
        `💾 [Server] Base de données: ${process.env.DB_NAME || "clubmanager"}`,
      );
