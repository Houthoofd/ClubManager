...
export const createMockPrisma = () => {
  ...
  return {
    utilisateurs: {
      findUnique: createMockFn((args: any) => {
        ...
        if (args.include?.genres)
          result.genres = genres.find((g: any) => g.id === user.genre_id);
        if (args.include?.grades)
          result.grades = grades.find((g: any) => g.id === user.grade_id);
        if (args.include?.status)
          result.status = status.find((s: any) => s.id === user.status_id);
      ...
      findMany: createMockFn((args?: any) => {
        ...
        if (args?.where?.first_name) {
          results = results.filter(
            (u: any) => u.first_name === args.where.first_name,
          );
        }
        if (args?.where?.last_name) {
          results = results.filter(
            (u: any) => u.last_name === args.where.last_name,
          );
        }
        if (args?.where?.email) {
        ...
        if (args?.include) {
          results = results.map((u: any) => {
            const result = { ...u };
            if (args.include.genres)
              result.genres = genres.find((g: any) => g.id === u.genre_id);
            if (args.include.grades)
              result.grades = grades.find((g: any) => g.id === u.grade_id);
            if (args.include.status)
              result.status = status.find((s: any) => s.id === u.status_id);
            if (args.include.plans_tarifaires) {
    ...
    genres: {
      ...
      findFirst: createMockFn((args: any) => {
        const genre = genres.find(
          (g) => g.genre_name === args.where?.genre_name,
        );
        return Promise.resolve(genre || null);
      }),
    },
    ...
    plans_tarifaires: {
      ...
      findFirst: createMockFn((args: any) => {
        const plan = plansTarifaires.find(
          (p) => p.nom_plan === args.where?.nom_plan,
        );
