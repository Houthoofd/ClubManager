...
const mockUtilisateurs = [
  ...
  { id: 3, first_name: 'Pierre', last_name: 'Durand', email: 'pierre@test.com' },
];
    ...
    inscriptions: {
      findMany: createMockFn((args?: any) => {
        ...
        if (args?.where?.utilisateur_id) {
          results = results.filter((i: any) => i.utilisateur_id === args.where.utilisateur_id);
        }
        if (args?.where?.cours_id) {
          results = results.filter((i: any) => i.cours_id === args.where.cours_id);
        }
        if (args?.where?.status_id !== undefined) {
          results = results.filter((i: any) => i.status_id === args.where.status_id);
        }
        if (args?.where?.date_inscription?.gte) {
          results = results.filter((i: any) => new Date(i.date_inscription) >= args.where.date_inscription.gte);
        }
        if (args?.where?.date_inscription?.lte) {
          results = results.filter((i: any) => new Date(i.date_inscription) <= args.where.date_inscription.lte);
        }
        ...
        if (args?.orderBy?.date_inscription) {
          results.sort((a: any, b: any) => {
            ...
            return args.orderBy.date_inscription === 'desc' ? dateB - dateA : dateA - dateB;
          });
        }
      ...
      findUnique: createMockFn((args: any) => {
        const inscription = inscriptions.find((i: any) => i.id === args.where.id);
      ...
      create: createMockFn((args: any) => {
        ...
        const existant = inscriptions.find(
          (i: any) => i.utilisateur_id === args.data.utilisateur_id && i.cours_id === args.data.cours_id
        );
        if (existant) {
        ...
        const newInscription = {
          ...
          users: utilisateurs.find((u: any) => u.id === args.data.utilisateur_id),
      ...
      update: createMockFn((args: any) => {
        const index = inscriptions.findIndex((i: any) => i.id === args.where.id);
        ...
        if (args.include?.users) {
          const user = mockUtilisateurs.find((u: any) => u.id === result.utilisateur_id);
          result.users = user;
        }
      ...
      groupBy: createMockFn((args: any) => {
        ...
        inscriptions.forEach((inscription: any) => {
          const key = args.by.map((field: string) => inscription[field]).join('-');
          if (!groupMap.has(key)) {
            const groupData: any = {};
