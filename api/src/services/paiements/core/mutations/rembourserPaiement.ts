...
export interface RembourserPaiementArgs {
  paiementId: number;
  motif?: string;
}

export async function rembourserPaiement(prisma: any, args: RembourserPaiementArgs) {
  ...
  if (paiement.statut !== 'validé') {
    throw new PaiementsError(
      `Impossible de rembourser un paiement avec le statut ${paiement.statut}`,
      'STATUT_INVALIDE'
    );
  }
  ...
  const paiementMisAJour = await prisma.paiements.update({
    ...
    data: {
      ...
      description: motif 
        ? `${paiement.description || ''} - Remboursement: ${motif}`.trim()
