/**
 * Mutation pour refuser un paiement
 */

import { PaiementsError } from '@clubmanager/types';

export interface RefuserPaiementArgs {
  paiementId: number;
  motif?: string;
}

export async function refuserPaiement(prisma: any, args: RefuserPaiementArgs) {
  ...
  if (paiement.statut !== 'en attente') {
    throw new PaiementsError(
      `Impossible de refuser un paiement avec le statut ${paiement.statut}`,
      'STATUT_INVALIDE'
    );
  }
  ...
  const paiementMisAJour = await prisma.paiements.update({
    ...
    data: {
      ...
      description: motif 
        ? `${paiement.description || ''} - Refus: ${motif}`.trim()
