...
export const mockCommandes = [
  {
    ...
    statut: 'en_attente',
    total: 150.00,
    articles: JSON.stringify([{ id: 1, nom: 'Article 1', prix: 75, quantite: 2 }]),
  ...
  },
  {
    ...
    statut: 'payee',
    total: 200.00,
    articles: JSON.stringify([{ id: 2, nom: 'Article 2', prix: 100, quantite: 2 }]),
  ...
  return {
    commandes: {
      ...
      findUnique: createMockFn(async (args: any) => {
        const commande = commandes.find(c => c.commande_id === args.where.commande_id);
      ...
      create: createMockFn(async (args: any) => {
        const newCommande = {
          ...
          ...args.data,
          articles: typeof args.data.articles === 'string' ? args.data.articles : JSON.stringify(args.data.articles),
      ...
      update: createMockFn(async (args: any) => {
        const index = commandes.findIndex(c => c.commande_id === args.where.commande_id);
