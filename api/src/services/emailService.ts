...
export class EmailService {
  ...
  async loadTemplate(templateName: string, variables: Record<string, string>): Promise<EmailTemplate> {
    try {
  ...
  // Email de bienvenue
  async sendWelcomeEmail(userEmail: string, userName: string, userId: string, tempPassword?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  async sendPasswordResetEmail(userEmail: string, userName: string, resetToken: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  async sendCourseNotification(userEmail: string, userName: string, courseInfo: {
    type: string;
    date: string;
    time: string;
    instructor?: string;
  ...
  async sendCourseReminder(userEmail: string, userName: string, courseInfo: {
    type: string;
    date: string;
    time: string;
    location?: string;
  ...
  async sendContactEmail(fromEmail: string, fromName: string, subject: string, message: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  async envoyerEmailBienvenue(
    ...
    try {
      ...
      console.log('🔧 [EmailService] Configuration validée - API Key:', 
        process.env.SENDGRID_API_KEY.substring(0, 15) + '...' + process.env.SENDGRID_API_KEY.slice(-5));
  ...
  async testerConfiguration(): Promise<{ success: boolean; details: any }> {
    try {
      ...
      if (!checks.fromName) recommendations.push('Optionnel: Ajouter EMAIL_FROM_NAME dans .env');
      if (!checks.frontendUrl) recommendations.push('Optionnel: Ajouter FRONTEND_URL dans .env');
  ...
  async envoyerEmailTest(emailTest: string): Promise<{ success: boolean; messageId?: string; error?: string; details?: any }> {
    console.log('🧪 [EmailService] Envoi d\'email de test vers:', emailTest);
    
    return this.envoyerEmailBienvenue(
      emailTest,
      'Test',
      'Utilisateur',
      'TEST_' + Date.now()
  ...
  async envoyerEmailPersonnalise(options: {
    ...
    try {
        ...
        const validAttachments = options.attachments
          ...
          .map(attachment => {
            ...
            if (Buffer.isBuffer(attachment.content)) {
              ...
              contentString = attachment.content as string;
            }
      ...
      console.log('📧 [EmailService] Envoi en cours...');
      const response = await Promise.race([sendPromise, timeoutPromise]) as [sgMail.ClientResponse, {}];
