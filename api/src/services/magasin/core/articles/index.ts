 ...
 */
export async function obtenirArticlesParCategories(prisma = defaultPrisma): Promise<ArticlesParCategorie> {
  ...
  try {
    ...
    console.log(`✅ [MagasinArticles] Articles groupés par ${Object.keys(mapCategories).length} catégories`);
    return mapCategories;
  } catch (error: any) {
    console.error('❌ [MagasinArticles] Erreur récupération par catégories:', error);
    throw error;
  }
}
 ...
 */
export async function modifierArticle(id: number, data: ArticleCreationData, prisma = defaultPrisma): Promise<ConfirmationResult> {
  ...
  try {
    ...
    console.error(`❌ [MagasinArticles] Erreur modification article ${id}:`, error);
    return {
      success: false,
      isConfirm: false,
 ...
 * Supprime un article
 */
export async function supprimerArticle(articleId: number, prisma = defaultPrisma): Promise<ConfirmationResult> {
  ...
  try {
    ...
    console.error(`❌ [MagasinArticles] Erreur suppression article ${articleId}:`, error);
    return {
      success: false,
      isConfirm: false,
