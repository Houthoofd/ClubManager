...
export const mockCours = [
  {
    id: 1,
    date_cours: new Date("2030-01-28"), // Bien dans le futur
    type_cours: "Karaté Débutant",
    heure_debut: "18:00",
    heure_fin: "19:30",
  ...
  {
    id: 2,
    date_cours: new Date("2030-01-30"), // Bien dans le futur
    type_cours: "Judo Avancé",
    heure_debut: "19:00",
    heure_fin: "20:30",
  ...
  {
    id: 3,
    date_cours: new Date("2030-01-29"), // Bien dans le futur
    type_cours: "Taekwondo Enfants",
    heure_debut: "14:00",
    heure_fin: "15:00",
  ...
  {
    id: 4,
    date_cours: new Date("2030-02-04"), // Bien dans le futur
    type_cours: "Karaté Débutant",
    heure_debut: "18:00",
    heure_fin: "19:30",
    cours_recurrent_id: 1,
    ...
    cours: {
      findMany: createMockFn((args?: any) => {
        ...
        if (args?.where?.date_cours?.gte) {
          results = results.filter(
            (c) =>
              new Date(c.date_cours).getTime() >=
              new Date(args.where.date_cours.gte).getTime(),
          );
        }
        if (args?.where?.date_cours?.lte) {
          results = results.filter(
            (c) =>
              new Date(c.date_cours).getTime() <=
              new Date(args.where.date_cours.lte).getTime(),
          );
        }
        ...
        if (args?.where?.inscriptions?.some) {
          ...
          results = results.filter((c) => {
            return inscriptions.some((i) => {
              ...
              if (
                someCondition.utilisateur_id &&
                i.utilisateur_id !== someCondition.utilisateur_id
              )
                return false;
              return true;
            });
        ...
        if (args?.orderBy?.date_cours) {
          results.sort((a, b) => {
            ...
            return args.orderBy.date_cours === "asc"
              ? dateA - dateB
              : dateB - dateA;
          });
        }
            ...
            inscriptions: inscriptions
              ...
              .map((i) => {
                ...
                if (args.include.inscriptions.include?.utilisateurs) {
                  insc.utilisateurs = utilisateurs.find(
    ...
    inscriptions: {
        ...
        if (args?.include) {
          results = results.map((i) => {
            ...
            if (args.include.utilisateurs) {
              result.utilisateurs = utilisateurs.find(
                (u) => u.id === i.utilisateur_id,
              );
            }
            return result;
      ...
      findFirst: createMockFn((args: any) => {
        ...
        if (args?.include) {
          ...
          if (args.include.utilisateurs) {
            result.utilisateurs = utilisateurs.find(
              (u) => u.id === inscription.utilisateur_id,
            );
          }
      ...
      updateMany: createMockFn((args: any) => {
        ...
        inscriptions.forEach((inscription, index) => {
          ...
          if (
            args?.where?.cours_id &&
            inscription.cours_id !== args.where.cours_id
          ) {
            matches = false;
          }
          if (
            args?.where?.utilisateur_id &&
            inscription.utilisateur_id !== args.where.utilisateur_id
          ) {
            matches = false;
          }
    ...
    cours_recurrent: {
      findMany: createMockFn((args?: any) => {
        ...
        if (args?.include?.cours_recurrent_professeur) {
          results = results.map((cr) => {
            const relations = coursRecurrentProfesseur.filter(
              (crp) => crp.cours_recurrent_id === cr.id,
            );
      ...
      findFirst: createMockFn((args: any) => {
        ...
        if (args?.where?.type_cours) {
          results = results.filter(
            (cr) => cr.type_cours === args.where.type_cours,
          );
        }
        if (args?.where?.heure_debut) {
          results = results.filter(
            (cr) => cr.heure_debut === args.where.heure_debut,
          );
        }
        if (args?.where?.heure_fin) {
          results = results.filter(
            (cr) => cr.heure_fin === args.where.heure_fin,
          );
        }
    ...
    cours_recurrent_professeur: {
      findMany: createMockFn((args?: any) => {
        ...
        if (args?.where?.professeur_id) {
          results = results.filter(
            (crp) => crp.professeur_id === args.where.professeur_id,
          );
        }
      ...
      deleteMany: createMockFn((args?: any) => {
        ...
        if (args?.where?.cours_recurrent_id) {
          const filtered = coursRecurrentProfesseur.filter(
            (crp) => crp.cours_recurrent_id !== args.where.cours_recurrent_id,
          );
