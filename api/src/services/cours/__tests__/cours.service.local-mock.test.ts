/**
 * Tests d'intégration pour le service Cours avec Mock Local
 * 
...
import * as coursQueries from '../core/cours/queries.js';
import * as inscriptionsQueries from '../core/inscriptions/queries.js';
import * as inscriptionsMutations from '../core/inscriptions/mutations.js';
import * as professeursМутations from '../core/professeurs/mutations.js';
import * as recurrentsQueries from '../core/recurrents/queries.js';
import * as recurrentsMutations from '../core/recurrents/mutations.js';
import * as statistiquesQueries from '../core/statistiques/queries.js';

describe('CoursService - Tests d\'intégration avec Mock Local', () => {
  let mockPrisma: any;
  ...
  describe('Queries - Cours', () => {
    it('devrait obtenir les cours pour un participant', async () => {
      const cours = await coursQueries.obtenirCoursPourParticipant(1, mockPrisma);
    ...
    it('devrait obtenir les cours par semaine', async () => {
      const cours = await coursQueries.obtenirCoursParSemaine(2, 5, mockPrisma);
    ...
    it('devrait obtenir tous les cours (admin)', async () => {
      const cours = await coursQueries.obtenirTousLesCours(mockPrisma);
    ...
    it('devrait obtenir les cours inscrits pour un utilisateur', async () => {
      const cours = await inscriptionsQueries.obtenirCoursInscritsParUtilisateur(1, mockPrisma);
  ...
  describe('Queries - Inscriptions', () => {
    it('devrait obtenir les utilisateurs d\'un cours', async () => {
      const result = await inscriptionsQueries.obtenirUtilisateursParCours(1, mockPrisma);
    ...
    it('devrait obtenir cours avec utilisateurs', async () => {
      const cours = await inscriptionsQueries.obtenirCoursAvecUtilisateurs(1, mockPrisma);
    ...
    it('devrait obtenir les participants d\'un cours', async () => {
      const result = await inscriptionsQueries.obtenirUtilisateursParticipantsParCours(1, mockPrisma);
  ...
  describe('Queries - Cours récurrents', () => {
    it('devrait obtenir tous les jours de cours', async () => {
      const jours = await recurrentsQueries.obtenirJoursDeCours(mockPrisma);
    ...
    it('devrait obtenir jours de cours par semaine', async () => {
      const jours = await recurrentsQueries.obtenirJoursDeCoursParSemaine(5, mockPrisma);
      
      expect(Array.isArray(jours)).toBe(true);
    });

    it('devrait obtenir un cours récurrent par ID', async () => {
      const cours = await recurrentsQueries.obtenirCoursRecurrentParId(1, mockPrisma);
    ...
    it('devrait trouver un cours récurrent par critères', async () => {
      const id = await recurrentsQueries.trouverCoursRecurrent(
        'mardi',
        'Karaté Débutant',
        '18:00',
        '19:30',
  ...
  describe('Queries - Statistiques', () => {
    it('devrait obtenir les semaines avec cours', async () => {
      const semaines = await statistiquesQueries.obtenirSemainesAvecCours(1, mockPrisma);
    ...
    it('devrait obtenir les statistiques de présence d\'un cours', async () => {
      const stats = await statistiquesQueries.obtenirStatistiquesPresenceCours(1, mockPrisma);
    ...
    it('devrait obtenir les statistiques de présence d\'un utilisateur', async () => {
      const stats = await statistiquesQueries.obtenirStatistiquesPresenceUtilisateur(2, mockPrisma);
  ...
  describe('Mutations - Inscriptions', () => {
    it('devrait vérifier si un utilisateur est inscrit', async () => {
      const result = await inscriptionsMutations.verifierInscriptionUtilisateur(1, 1, mockPrisma);
    ...
    it('devrait inscrire un utilisateur à un cours', async () => {
      const result = await inscriptionsMutations.inscrireUtilisateurAuCours({
        cours_id: 3,
        utilisateur_id: 1
      }, mockPrisma);
    ...
    it('devrait empêcher une double inscription', async () => {
      ...
      await inscriptionsMutations.inscrireUtilisateurAuCours({
        cours_id: 4,
        utilisateur_id: 1
      }, mockPrisma);
    ...
    it('devrait désinscrire un utilisateur', async () => {
      const result = await inscriptionsMutations.desinscrireUtilisateurDuCours({
        cours_id: 1,
        utilisateur_id: 1
      }, mockPrisma);
    ...
    it('devrait valider la présence d\'un utilisateur', async () => {
      const result = await inscriptionsMutations.validerPresenceUtilisateur({
        cours_id: 1,
        utilisateur_id: 2
      }, mockPrisma);
    ...
    it('devrait annuler/marquer absent un utilisateur', async () => {
      const result = await inscriptionsMutations.annulerPresenceUtilisateur({
        cours_id: 2,
        utilisateur_id: 2
      }, mockPrisma);
  ...
  describe('Mutations - Cours récurrents', () => {
    it('devrait ajouter un cours récurrent', async () => {
      const result = await recurrentsMutations.ajouterCoursRecurrent({
    ...
    it('devrait modifier un cours récurrent', async () => {
      const result = await recurrentsMutations.modifierCoursRecurrent({
    ...
    it('devrait supprimer un cours récurrent', async () => {
      ...
      const ajout = await recurrentsMutations.ajouterCoursRecurrent({
      ...
      if (ajout.success && ajout.data) {
        const result = await recurrentsMutations.supprimerCoursRecurrent(
    ...
    it('devrait supprimer un cours récurrent par jour', async () => {
      const result = await recurrentsMutations.supprimerCoursRecurrentParJour('mercredi', mockPrisma);
  ...
  describe('Mutations - Professeurs', () => {
    it('devrait associer des professeurs à un cours', async () => {
      const result = await professeursМутations.associerProfesseursAuCoursRecurrent(
        2,
        ['Judo Master'],
        mockPrisma
      );
    ...
    it('devrait gérer une liste vide de professeurs', async () => {
      const result = await professeursМутations.associerProfesseursAuCoursRecurrent(1, [], mockPrisma);
    ...
    it('devrait trouver le cours d\'un professeur', async () => {
      const context = await professeursМутations.trouverCoursAvecProfesseur(
        'Karate Sensei',
        'mardi',
        mockPrisma
      );
    ...
    it('devrait supprimer des professeurs d\'un cours', async () => {
      const result = await professeursМутations.supprimerProfesseursParNomEtJour({
    ...
    it('devrait supprimer des professeurs avec résolution auto', async () => {
      const result = await professeursМутations.supprimerProfesseursAvecResolution(
        ['Karate Sensei'],
        'mardi',
        undefined,
        mockPrisma
  ...
  describe('Cas limites et erreurs', () => {
    it('devrait gérer un cours_id inexistant', async () => {
      const result = await inscriptionsQueries.obtenirUtilisateursParCours(9999, mockPrisma);
    ...
    it('devrait gérer un utilisateur_id inexistant', async () => {
      const cours = await coursQueries.obtenirCoursPourParticipant(9999, mockPrisma);
