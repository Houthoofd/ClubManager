    ...
    informations: {
      findMany: createMockFn((args: any) => {
        ...
        if (args?.where) {
          if (args.where.status_id !== undefined) {
            results = results.filter(
              (i) => i.status_id === args.where.status_id,
            );
          }
        }
        ...
        if (args?.orderBy) {
          if (args.orderBy.date_creation === "desc") {
            results.sort(
              (a, b) => b.date_creation.getTime() - a.date_creation.getTime(),
            );
          } else if (args.orderBy.date_creation === "asc") {
            results.sort(
              (a, b) => a.date_creation.getTime() - b.date_creation.getTime(),
            );
          }
        }
      ...
      findFirst: createMockFn((args: any) => {
        let result = informations.find((i) => {
          if (args.where.id !== undefined && i.id !== args.where.id)
            return false;
          if (
            args.where.status_id !== undefined &&
            i.status_id !== args.where.status_id
          )
            return false;
          return true;
        });
