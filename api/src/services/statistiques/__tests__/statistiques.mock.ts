...
export const mockCours = [
  ...
  },
  {
    id: 2,
    type_cours: "Judo",
    date_cours: new Date("2024-06-16"),
...
export const mockPaiements = [
  {
    ...
    statut: "validé",
    periode_fin: new Date("2025-01-01"),
  },
  {
    id: 2,
    utilisateur_id: 2,
    montant: 75,
    date_paiement: new Date("2024-06-05"),
    statut: "confirmé",
    periode_fin: new Date("2025-01-01"),
  },
];
...
export const createMockPrisma = () => {
  return {
    utilisateurs: {
      ...
      findMany: createMockFn().mockResolvedValue(
        mockUtilisateurs.map((u) => ({
          ...u,
          plans_tarifaires: mockPlansTarifaires.find(
            (p) => p.id === u.plan_tarifaire_id,
          ),
          inscriptions: mockInscriptions.filter(
            (i) => i.utilisateur_id === u.id,
          ),
        })),
      ),
      findUnique: createMockFn().mockImplementation((args: any) => {
        return Promise.resolve(
          mockUtilisateurs.find((u) => u.id === args.where.id),
        );
      }),
    ...
    inscriptions: {
      ...
      findUnique: createMockFn().mockImplementation((args: any) => {
        return Promise.resolve(
          mockInscriptions.find((i) => i.id === args.where.id),
        );
      }),
    },
    paiements: {
      ...
      findUnique: createMockFn().mockImplementation((args: any) => {
        return Promise.resolve(
          mockPaiements.find((p) => p.id === args.where.id),
        );
      }),
    },
    professeurs: {
