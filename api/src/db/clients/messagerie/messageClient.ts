...
export interface MessageResult extends EmailResult {
  messageDbId?: number;
  templateUsed?: string;
}
...
export class MessageClient {
  ...
  async envoyerMessage(options: MessageClientOptions): Promise<MessageResult> {
    try {
  ...
  async notifierCours(options: NotificationCoursOptions): Promise<MessageResult> {
  ...
  private async saveMessageToDb(options: {
    utilisateurId: number;
    sujet: string;
    contenu: string;
    typeMessage: string;
  ...
  private async updateMessageStatus(messageId: number, success: boolean, messageIdSendGrid?: string, errorDetails?: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const sql = `
  ...
  async getHistoriqueMessages(utilisateurId: number, limit: number = 50): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const sql = `
        SELECT 
          id, 
  ...
  async processTemplate(content: string, variables: Record<string, string>) {
  ...
  async envoyerTest(email: string): Promise<MessageResult> {
  ...
  async envoyerGroupeEmail(options: {
    ...
    saveToDb?: boolean;
    adminUserId?: number;
  }): Promise<{ total: number; reussites: number; echecs: number; details: MessageResult[] }> {
  ...
  private async waitForDatabase(maxRetries: number = 5, delayMs: number = 1000): Promise<void> {
  ...
  private createEmailVerificationWithTokenTemplate(variables: {
    prenom: string;
    nom: string;
    email: string;
    userId: string;
