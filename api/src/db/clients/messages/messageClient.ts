...
import { UserIdGenerator } from '../../../utils/userIdGenerator.js';
...
export class MessageClient {
  ...
  async saveMessageToDatabase(messageData: MessageData): Promise<number> {
  ...
  async envoyerEmailBienvenue(data: WelcomeEmailData): Promise<EmailResult> {
    try {
      ...
      const emailResult = await this.emailService.envoyerEmailBienvenue(
        data.email,
        data.prenom,
        data.nom,
        data.userId
  ...
  async envoyerValidationEmail(data: ValidationEmailData): Promise<EmailResult> {
    try {
  ...
  async envoyerRecuperationUserId(data: RecoveryEmailData): Promise<EmailResult> {
    try {
  ...
  private async createValidationEmailTemplate(data: {
    prenom: string;
    userId: string;
...
Support : support@clubmanager.fr
  ...
  private async createUserIdRecoveryTemplate(data: {
    prenom: string;
    userId: string;
    loginUrl: string;
  }) {
...
Club Manager - Support : support@clubmanager.fr
  ...
  async getMessageHistory(utilisateurId: number, limit: number = 50): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const sql = `
        SELECT 
