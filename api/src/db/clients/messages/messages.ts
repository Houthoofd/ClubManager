...
${content}`;
  ...
  private genererHTMLEmail(titre: string, contenu: string, prenom: string): string {
    return `
      <!DOCTYPE html>
      <html>
      <head>
  ...
  private genererTextEmail(titre: string, contenu: string, prenom: string): string {
    return `
${titre}
...
© ${new Date().getFullYear()} Club Manager - Tous droits réservés
  ...
  async envoyerRappelPaiementAvecEmail(
    ...
    messageInterne: ConfirmationResult;
    emailEnvoye?: {
      success: boolean;
      messageId?: string;
      error?: string;
...
L'équipe du Club Manager`;
      ...
      const messageInterne = await this.envoyerMessagePersonnalise(
        senderId,
        receiverId,
        titre,
        contenu
      ...
      if (utilisateur.length > 0 && utilisateur[0].email) {
        ...
        try {
          ...
          const emailData = {
            ...
            html: this.genererHTMLEmailRappel(
              user.first_name, 
              echeanceId, 
              montant, 
              dateEcheance, 
            ...
            text: this.genererTextEmailRappel(
              user.first_name, 
              echeanceId, 
              montant, 
              dateEcheance, 
  ...
  private genererTokenPaiement(userId: number, echeanceId?: number): string {
  ...
  private genererHTMLEmailRappel(
    prenom: string, 
    echeanceId?: number, 
    montant?: number, 
    dateEcheance?: string,
  ...
  private genererTextEmailRappel(
    prenom: string, 
    echeanceId?: number, 
    montant?: number, 
    dateEcheance?: string,
