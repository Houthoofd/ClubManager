/**
 * @deprecated Cette classe est dépréciée et sera supprimée dans la version 3.0.0
 * Utilisez getCompteRepository() depuis './compte.repository.js' à la place
 *
 * @example
 * // ❌ Ancien (deprecated)
 ...
 *
 * // ✅ Nouveau (recommandé)
 * import { getCompteRepository } from './compte.repository';
 * const compteRepo = getCompteRepository();
 *
 * @see {@link ./compte.repository.ts} - Nouvelle implémentation
 * @see {@link ./docs/MIGRATION_GUIDE.md} - Guide de migration
 */
...
/**
 * @deprecated Utilisez getCompteRepository() à la place
 */
export class Compte {
  ...
  obtenirUnUtilisateurParSonNomEtPrenom(
    prenom: string,
    nom: string,
    ...
    try {
      const sql =
        "SELECT * FROM utilisateurs WHERE first_name = ? AND last_name = ?";
      ...
      console.error(
        "Erreur dans obtenirUnUtilisateurParSonNomEtPrenom:",
        error,
      );
      throw error;
    }
  ...
  obtenirInformationsUtilisateur = async (
    ...
    console.log(
      `[obtenirInformationsUtilisateur] Entrée - prenom: ${prenom}, nom: ${nom}`,
    );
    try {
      ...
      return new Promise<VerifyResultWithData>((resolve, reject) => {
        this.mysqlConnector.query(sql, values, (error, results) => {
          if (error) {
            console.error(
              `[obtenirInformationsUtilisateur] Erreur SQL:`,
              error,
            );
            reject(error); // Rejeter la promesse en cas d'erreur
          } else {
            console.log(
              `[obtenirInformationsUtilisateur] Résultats SQL:`,
              results,
            );
            if (results.length > 0) {
              const utilisateur = results[0]; // On prend le premier résultat si trouvé
              console.log(
                `[obtenirInformationsUtilisateur] Utilisateur trouvé:`,
                utilisateur,
              );
              resolve({
                isFind: true,
              ...
              console.log(
                `[obtenirInformationsUtilisateur] Aucun utilisateur trouvé pour ${prenom} ${nom}`,
              );
              resolve({
                isFind: false,
                message: "Aucun utilisateur trouvé",
  ...
  async mettreAJourMotDePasse(
    ...
    if (isCreation) {
      sql =
        'UPDATE utilisateurs SET password = ? WHERE id = ? AND (password IS NULL OR password = "")';
      values = [hash, id];
    } else {
  ...
  async mettreAJourUtilisateur(
    ...
    return new Promise((resolve) => {
      try {
        console.log(
          "🔍 [Compte] mettreAJourUtilisateur - ID:",
          utilisateurId,
          "Data:",
          updateData,
        );
   ...
   */
  async mettreAJourUtilisateurAvecConversion(
    utilisateurId: number,
    updateData: any,
  ): Promise<ConfirmationResult> {
    return new Promise(async (resolve) => {
      try {
        console.log(
          "🔍 [Compte] mettreAJourUtilisateurAvecConversion - ID:",
          utilisateurId,
          "Data:",
          updateData,
        );
        ...
        if (updateData.genres && isNaN(Number(updateData.genres))) {
          try {
            ...
            resolve({
              isConfirm: false,
              message: `Genre "${updateData.genres}" non trouvé`,
            });
            return;
          }
        }
        ...
        if (updateData.grades && isNaN(Number(updateData.grades))) {
          try {
            ...
            resolve({
              isConfirm: false,
              message: `Grade "${updateData.grades}" non trouvé`,
            });
            return;
          }
        }
        ...
        if (updateData.abonnement && isNaN(Number(updateData.abonnement))) {
          try {
            const abonnementId = await this.obtenirIdAbonnementParNom(
              updateData.abonnement,
            );
            convertedData.abonnement = abonnementId;
            console.log(
              "🔄 [Compte] Abonnement converti:",
              updateData.abonnement,
              "->",
              abonnementId,
            );
            ...
            resolve({
              isConfirm: false,
              message: `Abonnement "${updateData.abonnement}" non trouvé`,
            });
            return;
          }
        }
        ...
        if (updateData.status && isNaN(Number(updateData.status))) {
          try {
            const statusId = await this.obtenirIdStatusParNom(
              updateData.status,
            );
            convertedData.status = statusId;
            console.log(
              "🔄 [Compte] Status converti:",
              updateData.status,
              "->",
              statusId,
            );
            ...
            resolve({
              isConfirm: false,
              message: `Status "${updateData.status}" non trouvé`,
            });
            return;
          }
        }
        ...
        const result = await this.mettreAJourUtilisateur(
          utilisateurId,
          convertedData,
        );
        resolve(result);
        ...
        console.error(
          "❌ [Compte] Erreur dans mettreAJourUtilisateurAvecConversion:",
          error,
        );
        resolve({ isConfirm: false, message: `Erreur: ${error.message}` });
      }
