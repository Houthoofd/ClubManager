/**
 * 🧪 Tests d'Intégration - Audit
 *
 * Tests d'intégration avec base de données pour audit
 */
...
describe("Audit - Tests d'Intégration", () => {
  ...
  describe("Cycle de vie complet", () => {
    it("devrait créer, lire, mettre à jour et supprimer un audit", async () => {
      // CREATE
      ...
      expect(found?.name).toBe("Test Audit");

      // UPDATE
      const updated = await service.update(created.id, {
        name: "Updated Audit",
      });
      expect(updated.name).toBe("Updated Audit");
