 ...
 *
 * Tests d'intégration avec base de données pour documents
 */
...
describe("Documents - Tests d'Intégration", () => {
  ...
  describe("Cycle de vie complet", () => {
    it("devrait créer, lire, mettre à jour et supprimer un documents", async () => {
      // CREATE
      ...
      expect(found?.name).toBe("Test Documents");

      // UPDATE
      const updated = await service.update(created.id, {
        name: "Updated Documents",
      });
      expect(updated.name).toBe("Updated Documents");
