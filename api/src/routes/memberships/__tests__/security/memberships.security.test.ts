/**
 * 🔐 Tests de Sécurité - Memberships
 *
 * Tests de sécurité pour memberships
 */
...
describe("Memberships - Tests de Sécurité", () => {
  describe("Authentification", () => {
    ...
    it("devrait accepter les utilisateurs authentifiés", async () => {
      ...
      await expect(
        membershipsResolvers.Query.memberships({}, {}, authContext)
  ...
  describe("Autorisation", () => {
    ...
    it("devrait vérifier les permissions de création", async () => {
      ...
      await expect(
        membershipsResolvers.Mutation.createMemberships({}, { input: {} }, noPermContext)
      ).rejects.toThrow();
    });
    ...
    it("devrait vérifier les permissions de suppression", async () => {
      const noPermContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:memberships", "create:memberships"],
      } as any;

      await expect(
        membershipsResolvers.Mutation.deleteMemberships({}, { id: 1 }, noPermContext)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Injection et Validation", () => {
    it("devrait rejeter les IDs invalides", async () => {
      ...
      await expect(
        membershipsResolvers.Query.membership(
          {},
          { id: "invalid" as any },
          context
        )
    ...
    it("devrait échapper les caractères dangereux", async () => {
      ...
      const maliciousInput = {
        name: "'; DROP TABLE memberships; --",
      };

      const context = {
        user: { id: 1, role: "admin" },
        permissions: ["create:memberships"],
      } as any;

      // Ne devrait pas causer d'injection
      await expect(
        membershipsResolvers.Mutation.createMemberships(
          {},
