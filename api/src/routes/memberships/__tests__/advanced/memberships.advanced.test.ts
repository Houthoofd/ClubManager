/**
 * 🎯 Tests Avancés - Memberships
 *
 * Tests des cas limites et scénarios complexes pour memberships
 */

import { MembershipsService } from "../../core/services/memberships.service";
import { prisma } from "@/infrastructure/database/prisma-client";

describe("Memberships - Tests Avancés", () => {
  ...
  describe("Gestion des erreurs", () => {
    it("devrait gérer les erreurs de base de données", async () => {
      ...
      jest.spyOn(prisma.memberships, 'findMany').mockRejectedValue(
  ...
  describe("Performance", () => {
    it("devrait gérer de grandes quantités de données", async () => {
      ...
      jest.spyOn(prisma.memberships, 'findMany').mockResolvedValue(largeDataset as any);
