...
export const sendCustomEmailSchema = z
  .object({
    to: z.string().email("Adresse email invalide").trim(),
    subject: z
      .string()
...
export const sendTemplateEmailSchema = z.object({
  templateTitle: z.string().min(1, "Le titre du template est requis").trim(),
  to: z.string().email("Adresse email invalide").trim(),
...
export const sendWelcomeEmailSchema = z.object({
  ...
  userId: z.string().min(1, "L'identifiant utilisateur est requis").trim(),
  utilisateurId: z.number().int().positive().optional(),
});
...
export const confirmEmailSchema = z.object({
  token: z.string().min(1, "Le token est requis").trim(),
});
...
export type SendValidationEmailInput = z.infer<
  typeof sendValidationEmailSchema
>;
