...
export const sendMessageSchema = z.object({
  destinataires: z.array(z.number().int().positive())
    .min(1, 'Au moins un destinataire est requis')
    .max(1000, 'Maximum 1000 destinataires par envoi'),
  type_message_id: z.number().int().positive('L\'ID du type de message doit être un entier positif'),
...
export const sendPaymentReminderSchema = z.object({
  echeanceIds: z.union([
    ...
    if (Array.isArray(val)) {
      return val.filter(id => typeof id === 'number' && !isNaN(id) && id > 0);
    }
    if (typeof val === 'number' && !isNaN(val) && val > 0) {
      return [val];
    }
    return [];
  }).refine((ids) => ids.length > 0, 'Au moins une échéance valide est requise'),
...
export type CountUnreadMessagesInput = z.infer<typeof countUnreadMessagesSchema>;
export type ToggleMessageStatusInput = z.infer<typeof toggleMessageStatusSchema>;
