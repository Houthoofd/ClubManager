/**
 * 🧪 Tests d'Intégration - Statistics
 *
 * Tests d'intégration avec base de données pour statistics
 */
...
describe("Statistics - Tests d'Intégration", () => {
  ...
  describe("Cycle de vie complet", () => {
    it("devrait créer, lire, mettre à jour et supprimer un statistics", async () => {
      // CREATE
      ...
      expect(found?.name).toBe("Test Statistics");

      // UPDATE
      const updated = await service.update(created.id, {
        name: "Updated Statistics",
      });
      expect(updated.name).toBe("Updated Statistics");
