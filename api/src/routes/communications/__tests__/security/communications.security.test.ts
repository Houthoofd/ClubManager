/**
 * 🔐 Tests de Sécurité - Communications
 *
 * Tests de sécurité pour communications
 */
...
describe("Communications - Tests de Sécurité", () => {
  describe("Authentification", () => {
    ...
    it("devrait accepter les utilisateurs authentifiés", async () => {
      const authContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:communications"],
      } as any;

      // Ne devrait pas lever d'erreur d'authentification
      await expect(
        communicationsResolvers.Query.communications({}, {}, authContext)
  ...
  describe("Autorisation", () => {
    ...
    it("devrait vérifier les permissions de création", async () => {
      ...
      await expect(
        communicationsResolvers.Mutation.createCommunications({}, { input: {} }, noPermContext)
      ).rejects.toThrow();
    });
    ...
    it("devrait vérifier les permissions de suppression", async () => {
      const noPermContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:communications", "create:communications"],
      } as any;

      await expect(
        communicationsResolvers.Mutation.deleteCommunications({}, { id: 1 }, noPermContext)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Injection et Validation", () => {
    it("devrait rejeter les IDs invalides", async () => {
      ...
      await expect(
        communicationsResolvers.Query.communication(
          {},
          { id: "invalid" as any },
          context
        )
    ...
    it("devrait échapper les caractères dangereux", async () => {
      ...
      const maliciousInput = {
        name: "'; DROP TABLE communications; --",
      };

      const context = {
        user: { id: 1, role: "admin" },
        permissions: ["create:communications"],
      } as any;

      // Ne devrait pas causer d'injection
      await expect(
        communicationsResolvers.Mutation.createCommunications(
          {},
