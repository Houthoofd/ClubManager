...
describe('Auth Service - Tests de Sécurité', () => {
  ...
  describe('Protection contre les attaques par force brute', () => {
    it('devrait verrouiller le compte après plusieurs tentatives échouées', async () => {
      ...
      jest.spyOn(authService, 'authentifier').mockResolvedValue(mockLockedResult as any);
    ...
    it('devrait réinitialiser le compteur après une connexion réussie', async () => {
      ...
      const result = await authService.authentifier('test@example.com', 'correctpassword');
  ...
  describe('Protection des tokens de récupération', () => {
    ...
    it('devrait expirer les tokens après un certain temps', async () => {
      ...
      const result = await authService.verifierTokenRecuperation('expired-token-123');
    ...
    it('devrait invalider les tokens après utilisation', async () => {
      ...
      jest.spyOn(authService, 'verifierTokenRecuperation').mockResolvedValueOnce(mockFirstCheck as any);

      const firstCheck = await authService.verifierTokenRecuperation('valid-token-123');
      ...
      jest.spyOn(authService, 'reinitialiserMotDePasse').mockResolvedValue(mockReset as any);
      await authService.reinitialiserMotDePasse('valid-token-123', 'NewP@ssw0rd123');

      // Deuxième vérification : token devrait être invalide
      jest.spyOn(authService, 'verifierTokenRecuperation').mockResolvedValueOnce(null);

      const secondCheck = await authService.verifierTokenRecuperation('valid-token-123');
  ...
  describe('Protection contre l\'énumération des utilisateurs', () => {
    it('ne devrait pas révéler si un email existe lors de l\'authentification', async () => {
      ...
      jest.spyOn(authService, 'authentifier')
        .mockResolvedValueOnce(mockResultExisting as any)
        .mockResolvedValueOnce(mockResultNonExisting as any);

      const result1 = await authService.authentifier('existing@example.com', 'wrongpassword');
      const result2 = await authService.authentifier('nonexisting@example.com', 'wrongpassword');
    ...
    it('ne devrait pas révéler si un email existe lors de la récupération', async () => {
      ...
      jest.spyOn(authService, 'demanderRecuperationMotDePasse')
        .mockResolvedValue(mockResult as any);

      const result1 = await authService.demanderRecuperationMotDePasse('existing@example.com');
      const result2 = await authService.demanderRecuperationMotDePasse('nonexisting@example.com');
  ...
  describe('Validation des règles de mot de passe', () => {
    it('devrait rejeter les mots de passe courants', async () => {
      ...
      for (const pwd of commonPasswords) {
        ...
        jest.spyOn(authService, 'validerMotDePasse').mockResolvedValue(mockResult);
    ...
    it('devrait rejeter les mots de passe contenant des informations personnelles', async () => {
      const mockResult = {
        valid: false,
        errors: ['Le mot de passe ne doit pas contenir d\'informations personnelles'],
      };
  ...
  describe('Protection contre les attaques par timing', () => {
    it('devrait utiliser une comparaison de token à temps constant', async () => {
      ...
      jest.spyOn(authService, 'verifierTokenRecuperation')
