 ...
 */
export async function retirerProfesseur(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (type_cours && heure_debut && heure_fin) {
      ...
      const result = await client.supprimerProfesseursParNomEtJour(professeursValides, jour, coursContext);
      ...
      const result = await client.supprimerProfesseursParNomEtJourAvecResolution(professeursValides, jour, coursContext);
    ...
    res.status(500).json({
      success: false,
      message: 'Erreur serveur lors du retrait des professeurs.',
