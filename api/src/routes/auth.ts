...
router.post('/forgot-password', async (req: Request, res: Response) => {
  try {
    ...
    const users = await new Promise((resolve, reject) => {
      mysqlConnector.query(findUserQuery, [email], (error: any, results: any) => {
    ...
    try {
      await new Promise((resolve, reject) => {
        mysqlConnector.query(deleteOldTokensQuery, [user.id], (error: any, results: any) => {
          if (error) reject(error);
          else resolve(results);
        });
      });
    ...
    await new Promise((resolve, reject) => {
      mysqlConnector.query(saveTokenQuery, [user.id, token, expiresAt], (error: any, results: any) => {
        if (error) reject(error);
        else resolve(results);
      });
    });
    ...
    try {
      ...
      const emailResult = await emailClient.sendPasswordResetEmail(
        user.email,
        {
...
router.post('/reset-password', async (req: Request, res: Response) => {
  try {
    ...
    const tokenResults = await new Promise((resolve, reject) => {
      mysqlConnector.query(findTokenQuery, [token], (error: any, results: any) => {
        if (error) reject(error);
        else resolve(results);
      });
    }) as any[];
    ...
    await new Promise((resolve, reject) => {
      mysqlConnector.query(updatePasswordQuery, [hashedPassword, tokenData.utilisateur_id], (error: any, results: any) => {
        if (error) reject(error);
        else resolve(results);
      });
    });
    ...
    await new Promise((resolve, reject) => {
      mysqlConnector.query(deleteTokenQuery, [tokenData.utilisateur_id], (error: any, results: any) => {
        if (error) reject(error);
        else resolve(results);
      });
    });
...
router.get('/verify-token/:token', async (req: Request, res: Response) => {
  try {
    ...
    const results = await new Promise((resolve, reject) => {
      mysqlConnector.query(verifyTokenQuery, [token], (error: any, results: any) => {
        if (error) reject(error);
        else resolve(results);
      });
    }) as any[];
