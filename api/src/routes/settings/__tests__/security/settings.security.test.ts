/**
 * 🔐 Tests de Sécurité - Settings
 *
 * Tests de sécurité pour settings
 */
...
describe("Settings - Tests de Sécurité", () => {
  describe("Authentification", () => {
    ...
    it("devrait accepter les utilisateurs authentifiés", async () => {
      ...
      await expect(
        settingsResolvers.Query.settings({}, {}, authContext)
  ...
  describe("Autorisation", () => {
    ...
    it("devrait vérifier les permissions de création", async () => {
      ...
      await expect(
        settingsResolvers.Mutation.createSettings({}, { input: {} }, noPermContext)
      ).rejects.toThrow();
    });
    ...
    it("devrait vérifier les permissions de suppression", async () => {
      const noPermContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:settings", "create:settings"],
      } as any;

      await expect(
        settingsResolvers.Mutation.deleteSettings({}, { id: 1 }, noPermContext)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Injection et Validation", () => {
    it("devrait rejeter les IDs invalides", async () => {
      ...
      await expect(
        settingsResolvers.Query.setting(
          {},
          { id: "invalid" as any },
          context
        )
    ...
    it("devrait échapper les caractères dangereux", async () => {
      ...
      const maliciousInput = {
        name: "'; DROP TABLE settings; --",
      };

      const context = {
        user: { id: 1, role: "admin" },
        permissions: ["create:settings"],
      } as any;

      // Ne devrait pas causer d'injection
      await expect(
        settingsResolvers.Mutation.createSettings(
          {},
