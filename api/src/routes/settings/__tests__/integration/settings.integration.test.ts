 ...
 *
 * Tests d'intégration avec base de données pour settings
 */
...
describe("Settings - Tests d'Intégration", () => {
  ...
  describe("Cycle de vie complet", () => {
    it("devrait créer, lire, mettre à jour et supprimer un settings", async () => {
      // CREATE
      ...
      expect(found?.name).toBe("Test Settings");

      // UPDATE
      const updated = await service.update(created.id, {
        name: "Updated Settings",
      });
      expect(updated.name).toBe("Updated Settings");
