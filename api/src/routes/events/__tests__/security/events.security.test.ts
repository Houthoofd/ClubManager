/**
 * 🔐 Tests de Sécurité - Events
 *
 * Tests de sécurité pour events
 */
...
describe("Events - Tests de Sécurité", () => {
  describe("Authentification", () => {
    ...
    it("devrait accepter les utilisateurs authentifiés", async () => {
      ...
      await expect(
        eventsResolvers.Query.events({}, {}, authContext)
  ...
  describe("Autorisation", () => {
    ...
    it("devrait vérifier les permissions de création", async () => {
      ...
      await expect(
        eventsResolvers.Mutation.createEvents({}, { input: {} }, noPermContext)
      ).rejects.toThrow();
    });
    ...
    it("devrait vérifier les permissions de suppression", async () => {
      ...
      await expect(
        eventsResolvers.Mutation.deleteEvents({}, { id: 1 }, noPermContext)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Injection et Validation", () => {
    ...
    it("devrait échapper les caractères dangereux", async () => {
      ...
      await expect(
        eventsResolvers.Mutation.createEvents(
          {},
