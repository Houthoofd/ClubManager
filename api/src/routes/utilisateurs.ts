...
// ✅ SOFT DELETE v4.1 - Utilise la procédure stockée safe_delete_user (RGPD)
router.delete("/supprimer/:id", async (req: any, res: any) => {
  try {
    const utilisateurId = Number(req.params.id);
    const deletedBy = req.user?.id || null; // ID de l'admin qui effectue la suppression
    const reason = req.body.reason || "Suppression demandée"; // Raison de la suppression

    console.log(
      `[DELETE v4.1] Soft delete demandé pour id =`,
      utilisateurId,
      `par`,
      deletedBy,
    );
    ...
    if (!utilisateurId || isNaN(utilisateurId)) {
      console.log(`[DELETE v4.1] ID utilisateur invalide :`, req.params.id);
      return res
        .status(400)
    ...
    const utilisateurSimple = await client.obtenirUnUtilisateur(
      utilisateurId,
      false, // Seulement les actifs
    );
    console.log(
      `[DELETE v4.1] Résultat de obtenirUnUtilisateur :`,
      utilisateurSimple,
    );
    ...
    if (
      ...
      console.log(
        `[DELETE v4.1] Utilisateur introuvable ou déjà supprimé, id =`,
        utilisateurId,
      );
      return res
        .status(404)
    ...
    // ✅ Appel à la procédure stockée de soft delete (RGPD v4.1)
    const result = await client.softDeleteUser(
      utilisateurId,
      deletedBy,
      reason,
    );
    console.log(`[DELETE v4.1] Résultat de softDeleteUser :`, result);

    if (result.success) {
      console.log(`[DELETE v4.1] Soft delete réussi pour id =`, utilisateurId);
      res.status(200).json({
        isConfirm: true,
        success: true,
        message:
          result.message ||
          "Utilisateur supprimé (soft delete) avec succès. Données anonymisées selon RGPD.",
        action: "soft_deleted",
        details: result.details,
      });
    } else {
      console.log(
        `[DELETE v4.1] Le soft delete a échoué pour id =`,
        utilisateurId,
      );
      res.status(400).json({
        isConfirm: false,
    ...
    console.error(
      "[DELETE v4.1] Erreur lors du soft delete de l'utilisateur :",
      error,
    );
    res.status(500).json({
      isConfirm: false,
...
// ✅ RESTORE v4.1 - Restaurer un utilisateur supprimé (soft delete)
router.post("/restore/:id", async (req: any, res: any) => {
  try {
    const utilisateurId = Number(req.params.id);
    const restoredBy = req.user?.id || null; // ID de l'admin qui effectue la restauration

    console.log(
      `[RESTORE v4.1] Restauration demandée pour id =`,
      utilisateurId,
      `par`,
      restoredBy,
    );
    ...
    if (!utilisateurId || isNaN(utilisateurId)) {
      console.log(`[RESTORE v4.1] ID utilisateur invalide :`, req.params.id);
      return res
        .status(400)
    ...
    // ✅ Appel à la procédure stockée de restauration (v4.1)
    const result = await client.restoreUser(utilisateurId, restoredBy);
    console.log(`[RESTORE v4.1] Résultat de restoreUser :`, result);

    if (result.success) {
      console.log(
        `[RESTORE v4.1] Restauration réussie pour id =`,
        utilisateurId,
      );
      res.status(200).json({
        success: true,
        message: result.message || "Utilisateur restauré avec succès.",
        action: "restored",
        details: result.details,
      });
    } else {
      console.log(
        `[RESTORE v4.1] La restauration a échoué pour id =`,
        utilisateurId,
      );
      res.status(400).json({
        success: false,
        message:
          result.message ||
          "Échec de la restauration. L'utilisateur n'est peut-être pas supprimé ou les données sont anonymisées.",
      });
    }
  } catch (error: any) {
    console.error(
      "[RESTORE v4.1] Erreur lors de la restauration de l'utilisateur :",
      error,
    );
    res.status(500).json({
      success: false,
      message: "Erreur serveur lors de la restauration de l'utilisateur.",
      error: error.message,
    });
  }
});

// ✅ ARCHIVES v4.1 - Liste des utilisateurs archivés/supprimés
router.get("/archives", async (req: any, res: any) => {
  try {
    console.log(`[ARCHIVES v4.1] Liste des utilisateurs archivés demandée`);

    const client = new Utilisateurs();
    const archives = await client.obtenirUtilisateursArchives();

    if (archives.success && archives.data) {
      res.status(200).json({
        success: true,
        message: `${archives.data.length} utilisateur(s) archivé(s) trouvé(s).`,
        data: archives.data,
      });
    } else {
      res.status(200).json({
        success: true,
        message: "Aucun utilisateur archivé.",
        data: [],
      });
    }
  } catch (error: any) {
    console.error(
      "[ARCHIVES v4.1] Erreur lors de la récupération des archives :",
      error,
    );
    res.status(500).json({
      success: false,
      message:
        "Erreur serveur lors de la récupération des utilisateurs archivés.",
      error: error.message,
    });
  }
});
