 ...
 * 🎯 Tests Avancés - Gdpr
 *
 * Tests des cas limites et scénarios complexes pour gdpr
 */

import { GdprService } from "../../core/services/gdpr.service";
import { prisma } from "@/infrastructure/database/prisma-client";

describe("Gdpr - Tests Avancés", () => {
  ...
  describe("Gestion des erreurs", () => {
    it("devrait gérer les erreurs de base de données", async () => {
      ...
      jest.spyOn(prisma.gdpr, 'findMany').mockRejectedValue(
  ...
  describe("Performance", () => {
    it("devrait gérer de grandes quantités de données", async () => {
      ...
      jest.spyOn(prisma.gdpr, 'findMany').mockResolvedValue(largeDataset as any);
