/**
 * 🔐 Tests de Sécurité - Gdpr
 *
 * Tests de sécurité pour gdpr
 */
...
describe("Gdpr - Tests de Sécurité", () => {
  describe("Authentification", () => {
    ...
    it("devrait accepter les utilisateurs authentifiés", async () => {
      const authContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:gdpr"],
      } as any;

      // Ne devrait pas lever d'erreur d'authentification
      await expect(
        gdprResolvers.Query.gdpr({}, {}, authContext)
  ...
  describe("Autorisation", () => {
    ...
    it("devrait vérifier les permissions de création", async () => {
      ...
      await expect(
        gdprResolvers.Mutation.createGdpr({}, { input: {} }, noPermContext)
      ).rejects.toThrow();
    });
    ...
    it("devrait vérifier les permissions de suppression", async () => {
      const noPermContext = {
        user: { id: 1, role: "user" },
        permissions: ["read:gdpr", "create:gdpr"],
      } as any;

      await expect(
        gdprResolvers.Mutation.deleteGdpr({}, { id: 1 }, noPermContext)
      ).rejects.toThrow();
    });
  });
  ...
  describe("Injection et Validation", () => {
    ...
    it("devrait échapper les caractères dangereux", async () => {
      ...
      const maliciousInput = {
        name: "'; DROP TABLE gdpr; --",
      };

      const context = {
        user: { id: 1, role: "admin" },
        permissions: ["create:gdpr"],
      } as any;

      // Ne devrait pas causer d'injection
      await expect(
        gdprResolvers.Mutation.createGdpr(
          {},
