...
export class Paiements {
    ...
    async obtenirEcheancesUtilisateur(userId) {
        try {
        ...
        `;
                ...
                const echeancesFormatees = results.map((echeance) => ({
                    ...echeance,
    ...
    async marquerEcheancePayee(echeanceId, userId) {
        try {
      ...
      `;
            ...
            if (echeanceExistante.length === 0) {
                ...
                const echeanceSansUser = await this.queryAsync(`SELECT id, utilisateur_id, statut FROM echeances_paiements WHERE id = ?`, [echeanceId]);
    ...
    async envoyerEmailConfirmationPaiement(userId, montant, echeanceId) {
        try {
...
Merci de votre confiance !`;
            ...
            await messageService.envoyerMessagePersonnalise(1, // Système
    ...
    genererHTMLConfirmationPaiement(prenom, montant, echeanceId) {
        const montantFormate = new Intl.NumberFormat('fr-FR', {
            style: 'currency',
            currency: 'EUR'
        }).format(montant);
    ...
    genererTextConfirmationPaiement(prenom, montant, echeanceId) {
        const montantFormate = new Intl.NumberFormat('fr-FR', {
            style: 'currency',
            currency: 'EUR'
        }).format(montant);
...
© ${new Date().getFullYear()} Club Manager - Tous droits réservés
    ...
    async estPremierPaiement(userId) {
        try {
            const query = `
    ...
    async mettreAJourStatutUtilisateur(userId, nouveauStatutId = 2) {
        try {
            const query = `
        UPDATE utilisateurs 
    ...
    async mettreAJourDernierPaiementUtilisateur(userId) {
        try {
            const query = `
        UPDATE utilisateurs 
    ...
    async creerHistoriquePaiement(userId, montant, transactionId, methode) {
        try {
    ...
    async debugEcheancesUtilisateur(userId) {
        try {
