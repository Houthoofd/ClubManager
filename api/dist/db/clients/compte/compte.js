...
export class Compte {
   ...
   */
    async mettreAJourUtilisateur(utilisateurId, updateData) {
        return new Promise((resolve) => {
            try {
                ...
                this.mysqlConnector.query(query, values, (error, results) => {
                    if (error) {
     ...
     */
    async mettreAJourUtilisateurAvecConversion(utilisateurId, updateData) {
     ...
     */
    obtenirIdGenreParNom(genreName) {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id FROM genres WHERE genre_name = ?';
            this.mysqlConnector.query(sql, [genreName], (error, results) => {
                if (error) {
                    reject(error);
                }
                else if (results.length > 0) {
     ...
     */
    obtenirIdGradeParNom(gradeName) {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id FROM grades WHERE grade_id = ?';
            this.mysqlConnector.query(sql, [gradeName], (error, results) => {
                if (error) {
                    reject(error);
                }
                else if (results.length > 0) {
     ...
     */
    obtenirIdAbonnementParNom(abonnementName) {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id FROM plans_tarifaires WHERE nom_plan = ?';
            this.mysqlConnector.query(sql, [abonnementName], (error, results) => {
                if (error) {
                    reject(error);
                }
                else if (results.length > 0) {
     ...
     */
    obtenirIdStatusParNom(statusName) {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT id FROM status WHERE nom_role = ?';
            this.mysqlConnector.query(sql, [statusName], (error, results) => {
                if (error) {
                    reject(error);
                }
                else if (results.length > 0) {
