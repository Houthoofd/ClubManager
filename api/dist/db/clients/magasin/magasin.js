...
export class Magasin {
    ...
    async ajouterCommande(data) {
        ...
        const articlesAvecTailleIdInvalide = articles.filter((article) => {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                this.mysqlConnector.beginTransaction((err, connection) => {
                    ...
                    this.mysqlConnector.query(sqlInsertCommande, [utilisateur_id, total || 0, date, statut || 'en_attente'], (errCommande, resCommande) => {
                        if (errCommande) {
                        ...
                        try {
                            const valeursArticles = articles.map((article, index) => {
                  ...
                  VALUES ?
                `;
                            this.mysqlConnector.query(sqlInsertArticles, [valeursArticles], (errArticles) => {
                                if (errArticles) {
                                    this.mysqlConnector.rollback(connection);
    ...
    `;
        return new Promise((resolve, reject) => {
            this.mysqlConnector.query(sql, [quantite, articleId, tailleId], (error, result) => {
                if (error) {
                    reject({
                        isConfirm: false,
    ...
    creerCommande(utilisateur_id, articles, total, date, statut) {
        return new Promise(async (resolve, reject) => {
