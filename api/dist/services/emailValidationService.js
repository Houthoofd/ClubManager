...
export class EmailValidationService {
    mysqlConnector;
    initialized = false;
    emailService = null; // Déclaré comme any temporairement
    constructor() {
    ...
    async waitForDatabase(maxRetries = 5, delayMs = 500) {
    ...
    async createValidationTokensTable() {
    ...
    async createMessagesPersonnalisesTable() {
        return new Promise((resolve, reject) => {
            const sql = `
    ...
    async createEmailConfirmationToken(utilisateurId) {
    ...
    async createPasswordSetupToken(utilisateurId) {
        const token = this.generateSecureToken();
        const expiresAt = new Date();
    ...
    async sendValidationEmail(utilisateurId) {
        await this.ensureInitialized();
        try {
    ...
    async createEmailValidationTokenWithUserId(utilisateurId, userId) {
        await this.ensureInitialized();
    ...
    async validateEmailTokenWithHash(token, userId) {
        await this.ensureInitialized();
        try {
    ...
    async sendValidationEmailWithUserId(options) {
        ...
        try {
            ...
            const { token: validationToken, hashedToken } = await this.createEmailValidationTokenWithUserId(options.utilisateurId, options.userId);
    ...
    `;
        return new Promise((resolve, reject) => {
            ...
            this.mysqlConnector.query(createTableSql, [], (error) => {
                if (error) {
    ...
    async cleanupExpiredTokens() {
      ...
      `;
            ...
            this.mysqlConnector.query(sql, [], (error, result) => {
                if (error) {
    ...
    async ensureValidationTokensTableStructure() {
        return new Promise((resolve, reject) => {
