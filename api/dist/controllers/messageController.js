...
export class MessageController {
    ...
    async envoyerEmailBienvenue(req, res) {
        try {
    ...
    async envoyerEmailValidation(req, res) {
        try {
    ...
    async obtenirStatistiquesEmails(req, res) {
        try {
    ...
    async envoyerNotificationCours(req, res) {
        try {
            const { email, prenom, nom, coursInfo, typeNotification = 'inscription', utilisateurId, saveToDb = true } = req.body;
            ...
            const template = templates[typeNotification];
            if (!template) {
                return res.status(400).json({
                    success: false,
            ...
            if (saveToDb && dbMessageId) {
                await messageClient.updateMessageStatus(dbMessageId, emailResult.success ? 'sent' : 'failed', emailResult.messageId, emailResult.error);
            }
    ...
    async envoyerEmailGroupe(req, res) {
        try {
            const { destinataires, subject, message, isHtml = false, adminUserId, saveToDb = true } = req.body;
