...
router.get('/', async (req, res) => {
    try {
        const typesMessages = await queryAsync('SELECT * FROM types_messages_personnalises ORDER BY title', []);
        res.json({
            success: true,
            data: typesMessages
        });
...
router.post('/types', verifyToken, async (req, res) => {
    try {
        ...
        await queryAsync('INSERT INTO types_messages_personnalises (title, content) VALUES (?, ?)', [title, content]);
        res.json({
            success: true,
...
router.put('/types/:id', verifyToken, async (req, res) => {
    try {
        ...
        await queryAsync('UPDATE types_messages_personnalises SET title = ?, content = ?, updated_at = NOW() WHERE id = ?', [title, content, id]);
        res.json({
            success: true,
...
router.delete('/types/:id', verifyToken, async (req, res) => {
    try {
        const { id } = req.params;
        await queryAsync('DELETE FROM types_messages_personnalises WHERE id = ?', [id]);
        res.json({
            success: true,
...
router.get('/recus/:userId', verifyToken, async (req, res) => {
    try {
        ...
        const users = await queryAsync('SELECT id FROM utilisateurs WHERE userId = ?', [userId]);
        if (!users.length) {
        ...
        const messagesRecus = await queryAsync(`SELECT 
        mp.id,
        mp.contenu,
...
router.delete('/recus/:messageId', verifyToken, async (req, res) => {
    try {
        const { messageId } = req.params;
        await queryAsync('DELETE FROM messages_personnalises WHERE id = ?', [messageId]);
        res.json({
            success: true,
...
router.post('/envoie', verifyToken, async (req, res) => {
    try {
        ...
        const typeMessages = await queryAsync('SELECT content FROM types_messages_personnalises WHERE id = ?', [type_message_id]);
        ...
        for (const destinataireId of destinataires) {
            await queryAsync('INSERT INTO messages_personnalises (utilisateur_id, contenu) VALUES (?, ?)', [destinataireId, contenu]);
        }
        res.json({
            success: true,
