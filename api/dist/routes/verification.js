...
router.get('/magasin/article', async (req, res) => {
    const { nom } = req.query;
    if (!nom)
        return res.status(400).json({ message: "Nom de l'article requis." });
    try {
        const client = new Verifiation();
...
router.get('/magasin/article/categorie', async (req, res) => {
...
router.post('/verifier-professeurs', async (req, res) => {
