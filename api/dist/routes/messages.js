...
router.post('/envoie', async (req, res) => {
    ...
    try {
        ...
        const results = await Promise.all(destinataires.map(async (userId) => {
            return await client.envoyerMessagePersonnalise(req.user?.id || 1, // ID de l'expéditeur (depuis le token JWT ou défaut)
            userId, typeMessage.title, typeMessage.content);
        }));
        return res.json({
            success: true,
