...
const generateUniqueCommandeId = (userId) => {
...
router.get('/commande/:uniqueId/verify', async (req, res) => {
    try {
...
const generateSequentialCommandeNumber = async () => {
    try {
...
router.post('/commandes/ajouter', async (req, res) => {
    try {
        ...
        if (req.body) {
            ...
            const cacheKey = `${data.utilisateur_id}-${data.total}-${JSON.stringify((data.articles || []).map((a) => ({
                id: a.article_id,
                taille: a.taille, // Utiliser uniquement la propriété 'taille' qui existe
                quantite: a.quantite
            })))}`;
...
router.get('/commande/:commandeId/payment-intent', async (req, res) => {
    try {
...
const envoyerEmailConfirmationCommande = async (commandeData, utilisateurEmail, utilisateurNom) => {
    try {
        const emailClient = new EmailClient();
        // Générer la liste des articles pour le template
        const articlesFormatted = commandeData.articles.map((article) => ({
...
const recupererDonneesUtilisateur = async (utilisateurId) => {
    try {
        const paiements = new Paiements();
        const userQuery = `
