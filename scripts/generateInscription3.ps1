...
# Obtenir les ids des cours valides dans la base de données (en supposant que tu as une base de données MySQL ou autre)
# Pour MySQL : SELECT id FROM cours;
# Remplace cette partie par une API ou une requête si nécessaire.

# Supposons que tu as récupéré tous les ids valides dans la base de données
$coursIdsValides = 1..417 # Cela suppose que tu as des cours avec ids de 1 à 417
...
for ($i = 1; $i -le 152; $i++) {
    ...
    for ($j = 1; $j -le 417; $j++) {
        # Génère un utilisateur_id
        $utilisateurId = $i
        
        # Choisir un cours valide parmi ceux récupérés
        $coursId = $coursIdsValides | Get-Random

        # Choisir un cours aléatoire parmi ceux récupérés
        $randomCours = $coursDates | Where-Object { $_.id -eq $coursId }
