...
async function handlePaymentSuccess(
  ...
  try {
    ...
    const { emailService } =
      await import("../../services/operations/communication/email.service.js");
    ...
    try {
      ...
      console.log(
        `📝 [Webhook] PaymentIntent traité avec succès: ${paymentIntent.id}`,
      );
    } catch (error: any) {
      console.warn(
        `⚠️ [Webhook] Erreur traitement PaymentIntent:`,
        error.message,
      );
    }
    ...
    if (echeanceId && utilisateurId) {
      try {
        console.log(
          `🎯 [Webhook] Mise à jour échéance ${echeanceId} dans echeances_paiements`,
        );

        // NOUVEAU: Mettre à jour directement la table echeances_paiements
        // Mettre à jour l'échéance avec Prisma
        const echeanceUpdateResult =
          await prismaClient.echeancePaiement.updateMany({
            where: {
        ...
        console.log(
          `📊 [Webhook] Résultat mise à jour échéance:`,
          echeanceUpdateResult,
        );

        if (echeanceUpdateResult.count > 0) {
          console.log(
            `✅ [Webhook] Échéance ${echeanceId} mise à jour: statut = 'payé', date_paiement = aujourd'hui`,
          );

          // Vérifier si c'est le premier paiement de l'utilisateur
          // Vérifier s'il s'agit du premier paiement
          const premierPaiement =
            (await prismaClient.echeancePaiement.count({
              where: {
                utilisateurId: parseInt(utilisateurId),
                statut: "payé",
              },
          ...
          if (premierPaiement) {
            console.log(
              `🎉 [Webhook] Premier paiement détecté pour utilisateur ${utilisateurId}`,
            );
          ...
          try {
            ...
            if (utilisateur && utilisateur.email) {
              console.log(
                `📧 [Webhook] Envoi confirmation paiement à ${utilisateur.email}`,
              );
              ...
              console.log(
                `✅ [Webhook] Email de confirmation envoyé à ${utilisateur.email}`,
              );
            } else {
              console.warn(
                `⚠️ [Webhook] Aucun email trouvé pour l'utilisateur ${utilisateurId}`,
              );
            }
          } catch (emailError: any) {
            console.error(
              `❌ [Webhook] Erreur envoi email confirmation:`,
              emailError.message,
            );
          }
        } else {
          console.warn(
            `⚠️ [Webhook] Échéance ${echeanceId} NON mise à jour - peut-être déjà payée ou inexistante`,
          );
          ...
          console.log(
            `🔍 [Webhook] État actuel échéance ${echeanceId}:`,
            currentEcheance,
          );
        }
      } catch (echeanceError: any) {
        console.error(
          `❌ [Webhook] Erreur mise à jour échéance ${echeanceId}:`,
          echeanceError.message,
        );
      }
    } else {
      console.warn(
        `⚠️ [Webhook] Métadonnées manquantes - Pas de mise à jour d'échéance`,
      );
      console.warn(
        `⚠️ [Webhook] echeanceId: ${echeanceId}, utilisateurId: ${utilisateurId}`,
      );
    }
    ...
    try {
      ...
      if (!utilisateurId) {
        console.warn(
          "⚠️ [Webhook] utilisateurId manquant pour enregistrement historique",
        );
        return;
      }
      ...
      console.log(
        `📝 [Webhook] Enregistrement paiement réussi pour ${paymentIntent.id}`,
      );
      console.log(
        `📝 [Webhook] Historique paiement enregistré: montant=${paymentIntent.amount / 100}, utilisateur=${utilisateurId}`,
      );
    } catch (enregistrementError: any) {
      console.error(
        `❌ [Webhook] Erreur enregistrement historique:`,
        enregistrementError.message,
      );
    }

    console.log(
      `🎉 [Webhook] Traitement complet du paiement ${paymentIntent.id} terminé avec succès`,
    );
  } catch (error: any) {
    console.error(
      "❌ [Webhook] Erreur lors du traitement du paiement réussi:",
      error,
    );
...
async function handlePaymentFailed(
  ...
  try {
    ...
    try {
      ...
      console.log(
        `📝 [Webhook] Paiement échoué enregistré: ${paymentIntent.id}`,
      );
      console.log(`📝 [Webhook] PaymentIntent marqué comme échoué`);
    } catch (error: any) {
      console.warn(
        `⚠️ [Webhook] Erreur mise à jour PaymentIntent (peut-être pas en base):`,
        error.message,
      );
    }
    ...
    if (utilisateurId) {
      try {
        ...
        if (utilisateur && utilisateur.email) {
          console.log(
            `📧 [Webhook] Envoi notification échec à ${utilisateur.email}`,
          );
          ...
          console.log(
            `✅ [Webhook] Email d'échec envoyé à ${utilisateur.email}`,
          );
        }
      } catch (emailError: any) {
        console.error(
          `❌ [Webhook] Erreur envoi email échec:`,
          emailError.message,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `📝 [Webhook] Échec paiement enregistré pour ${paymentIntent.id}`,
      );
      console.log(
        `📝 [Webhook] Enregistrement échec créé: montant=${paymentIntent.amount / 100}, utilisateur=${utilisateurId}`,
      );
    } catch (enregistrementError: any) {
      console.error(
        `❌ [Webhook] Erreur enregistrement échec:`,
        enregistrementError.message,
      );
    }

    console.log(
      `📝 [Webhook] Traitement complet de l'échec ${paymentIntent.id} terminé`,
    );
  } catch (error: any) {
    console.error(
      "❌ [Webhook] Erreur lors du traitement de l'échec de paiement:",
      error,
    );
  }
}
...
async function handleCheckoutCompleted(
  ...
  if (echeanceId && userId) {
    try {
      console.log(
        `✅ [Webhook] Échéance ${echeanceId} payée par user ${userId} via checkout ${session.id}`,
      );
...
async function handleInvoicePaymentSucceeded(
  ...
  if (subscriptionId) {
    try {
      console.log(
        `🔄 [Webhook] Renouvellement abonnement ${subscriptionId} pour client ${customerId}`,
      );
...
async function handleInvoicePaymentFailed(
  ...
  try {
    console.log(
      `⚠️ [Webhook] Échec paiement abonnement ${subscriptionId} pour client ${customerId}`,
    );
...
async function handleSubscriptionChange(
  ...
  console.log(
    `🔔 [Webhook] Changement abonnement (${eventType}):`,
    subscription.id,
  );
  ...
  try {
    switch (eventType) {
      case "customer.subscription.created":
        console.log(
          `➕ [Webhook] Nouvel abonnement ${subscription.id} pour client ${customerId}`,
        );
      ...
      case "customer.subscription.updated":
        console.log(
          `✏️ [Webhook] Abonnement ${subscription.id} modifié pour client ${customerId}`,
        );
      ...
      case "customer.subscription.deleted":
        console.log(
          `❌ [Webhook] Abonnement ${subscription.id} annulé pour client ${customerId}`,
        );
    ...
    console.error(
      "❌ [Webhook] Erreur traitement changement abonnement:",
      error,
    );
  }
}
