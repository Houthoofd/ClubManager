...
router.get('/summary', async (req: Request, res: Response) => {
  try {
    ...
    sendError(
      res,
      'INTERNAL_ERROR',
      error instanceof Error ? error.message : 'Failed to get inventory summary'
    );
  }
});
...
router.get('/alerts', async (req: Request, res: Response) => {
  try {
    ...
    const alerts = await inventoryService.getLowStockAlerts(threshold, tenantId);
...
router.get('/out-of-stock', async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const products = await inventoryService.getOutOfStockProducts(tenantId);
    ...
    sendError(
      res,
      'INTERNAL_ERROR',
      error instanceof Error ? error.message : 'Failed to get out of stock products'
    );
  }
});
...
router.post('/:productId/add', async (req: Request, res: Response) => {
  try {
    ...
    if (!quantity || quantity <= 0) {
      return sendError(res, 'VALIDATION_ERROR', 'Valid quantity is required', { field: 'quantity' }, 400);
    }
...
router.post('/bulk-update', async (req: Request, res: Response) => {
  try {
    ...
    if (!updates || !Array.isArray(updates)) {
      return sendError(res, 'VALIDATION_ERROR', 'Updates array is required', { field: 'updates' }, 400);
    }

    const result = await inventoryService.bulkUpdate(updates, tenantId, userId);

    sendSuccess(res, result, `Bulk update completed: ${result.successCount} successful, ${result.failureCount} failed`);
  } catch (error) {
    if (error instanceof ShopValidationError) {
      return sendError(res, 'VALIDATION_ERROR', error.message, { field: error.field }, 400);
    }
    ...
    sendError(
      res,
      'INTERNAL_ERROR',
      error instanceof Error ? error.message : 'Failed to bulk update inventory'
    );
  }
});
...
router.post('/reserve', async (req: Request, res: Response) => {
  try {
    ...
    const result = await inventoryService.reserveStock(items, tenantId, userId, orderId);
...
router.post('/release', async (req: Request, res: Response) => {
  try {
    ...
    const result = await inventoryService.releaseStock(items, tenantId, userId, orderId);
