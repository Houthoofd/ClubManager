...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const stats = await productService.getStatistics(tenantId);
    ...
    sendError(
      res,
      'INTERNAL_ERROR',
      error instanceof Error ? error.message : 'Failed to get product statistics'
    );
  }
});
...
router.get('/low-stock', async (req: Request, res: Response) => {
  try {
    ...
    const products = await productService.getLowStock(threshold, tenantId);
    ...
    sendError(
      res,
      'INTERNAL_ERROR',
      error instanceof Error ? error.message : 'Failed to get low stock products'
    );
  }
});
...
router.get('/out-of-stock', async (req: Request, res: Response) => {
  try {
    const tenantId = getTenantId(req);
    const products = await productService.getOutOfStock(tenantId);
...
router.patch('/:id/stock', async (req: Request, res: Response) => {
  try {
    ...
    if (stock === undefined || stock === null) {
      return sendError(res, 'VALIDATION_ERROR', 'Stock quantity is required', { field: 'stock' }, 400);
    }

    const product = await productService.updateStock(id, stock, tenantId, userId);
