...
-- Création de la table tailles
CREATE TABLE tailles (
    id INT AUTO_INCREMENT PRIMARY KEY,
    nom VARCHAR(20) NOT NULL  -- Exemple: "S", "M", "L", "XL"
);

-- Création de la table categories (si tu en as une)
...
CREATE TABLE articles (
    ...
    categorie_id INT,  -- Référence à la table des catégories (si tu as une table de catégories)
    
    -- Clé étrangère vers une table de catégories (si tu en as une)
...
-- Création de la table commande_articles
...
-- Créer la table des cours récurrents (doit être créé avant `cours`)
CREATE TABLE cours_recurrent (
    id INT AUTO_INCREMENT PRIMARY KEY,
    type_cours VARCHAR(255),        -- Type de cours (ex: Grappling, JJB)
    jour_semaine INT,               -- Jour de la semaine (1 pour dimanche, 2 pour lundi, etc.)
    heure_debut TIME,               -- Heure de début
    heure_fin TIME,                 -- Heure de fin
    active BOOLEAN DEFAULT TRUE     -- Si la récurrence est active ou non
);

-- Créer la table des cours (fait référence à `cours_recurrent`)
CREATE TABLE cours (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
-- Créer la table des inscriptions avec ON DELETE CASCADE (fait référence à `cours` et `utilisateurs`)
...
-- Créer la table des professeurs
...
-- Créer la table des cours récurrents-professeurs (fait référence à `cours_recurrent` et `professeur`)
...
-- 🗓️ Étape 1 : Définir les types de cours récurrents
INSERT INTO cours_recurrent (jour_semaine, type_cours)
VALUES
    (1, 'Grappling'),  -- Dimanche
    (2, 'JJB'),        -- Lundi
    (5, 'JJB'),        -- Jeudi
    (7, 'Grappling');  -- Samedi
...
-- 3. Générer les dates valides entre saison_start et season_end, uniquement les jours d'entraînement
...
WHERE DAYOFWEEK(date_generated) IN (1, 2, 5, 7);

-- 4. Vérifier ce qui est dans dates_valides avant insertion
SELECT date_cours, jour_semaine FROM dates_valides ORDER BY date_cours LIMIT 10;

-- 5. Insérer dans cours, en s’assurant que la correspondance est bonne
...
ORDER BY dv.date_cours;

-- 6. Vérifier les premières entrées insérées
SELECT * FROM cours ORDER BY date_cours LIMIT 20;
...
CREATE TABLE paiements (
    ...
    statut ENUM('validé', 'en attente', 'pas encore effectué') DEFAULT 'pas encore effectué',  -- Statut du paiement
    ...
    -- Contrainte UNIQUE pour un paiement unique par période pour chaque utilisateur
    UNIQUE (utilisateur_id, periode_debut)
);
...
CREATE TABLE messages (
  id INT AUTO_INCREMENT PRIMARY KEY,
  sender_id INT NOT NULL,         -- Qui envoie
  receiver_id INT DEFAULT NULL,   -- À qui ? (null si c'est pour un groupe)
  groupe_id INT DEFAULT NULL,     -- Pour les chats de groupe
...
CREATE TABLE message_status (
  ...
  utilisateur_id INT NOT NULL,  -- L'utilisateur qui a vu ou non le message
  status ENUM('vu', 'non vu') DEFAULT 'non vu',  -- Statut du message
...
insert into utilisateurs (id, first_name, last_name, email, genre_id, date_of_birth, status_id, grade_id, abonnement_id) values (151, 'Emmanuel', 'Snow', 'Emanuel.snow@gmail.com', 1, '1969-04-7', 5, 13, 2);
insert into utilisateurs (id, first_name, last_name, email, genre_id, date_of_birth, status_id, grade_id, abonnement_id) values (152, 'Robin', 'Rivière', 'Robin.riviere@gmail.com', 1, '1975-05-10', 5, 13, 2);
insert into utilisateurs (id, first_name, last_name, email, genre_id, date_of_birth, status_id, grade_id, abonnement_id) values (153, 'Rachid', 'Belhoui', 'rachid/belhoui@gmail.com', 1, '1980-06-12', 5, 13, 2);
insert into utilisateurs (id, first_name, last_name, email, genre_id, date_of_birth, status_id, grade_id, abonnement_id) values (154, 'Benoit', 'Houthoofd', 'houthoofd.benoit48@gmail.com', 1, '1993-08-12', 4, 13, 2);


-- Insertion des paiements pour 151 utilisateurs
INSERT INTO paiements (utilisateur_id, montant, date_paiement, statut, abonnement_id, periode_debut, periode_fin)
VALUES 
    (1, 25.00, '2025-01-01', 'validé', 1, '2025-01-01', '2025-01-31'),  -- Paiement mensuel
    (2, 100.00, '2025-01-15', 'en attente', 2, '2025-01-01', '2025-03-31'),  -- Paiement trimestriel
    (3, 300.00, '2025-02-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (4, 25.00, '2025-02-01', 'validé', 1, '2025-02-01', '2025-02-28'),  -- Paiement mensuel pour février
    (5, 100.00, '2025-03-01', 'pas encore effectué', 2, '2025-04-01', '2025-06-30'),  -- Paiement trimestriel non effectué
    (6, 300.00, '2025-03-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel pour un autre utilisateur
    (7, 25.00, '2025-03-15', 'validé', 1, '2025-03-01', '2025-03-31'),  -- Paiement mensuel
    (8, 100.00, '2025-04-01', 'en attente', 2, '2025-04-01', '2025-06-30'),  -- Paiement trimestriel
    (9, 300.00, '2025-05-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (10, 25.00, '2025-05-01', 'validé', 1, '2025-05-01', '2025-05-31'),  -- Paiement mensuel
    (11, 100.00, '2025-06-01', 'pas encore effectué', 2, '2025-07-01', '2025-09-30'),  -- Paiement trimestriel non effectué
    (12, 300.00, '2025-07-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (13, 25.00, '2025-07-01', 'validé', 1, '2025-07-01', '2025-07-31'),  -- Paiement mensuel
    (14, 100.00, '2025-08-01', 'en attente', 2, '2025-08-01', '2025-10-31'),  -- Paiement trimestriel
    (15, 300.00, '2025-09-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (16, 25.00, '2025-09-01', 'validé', 1, '2025-09-01', '2025-09-30'),  -- Paiement mensuel
    (17, 100.00, '2025-10-01', 'en attente', 2, '2025-10-01', '2025-12-31'),  -- Paiement trimestriel
    (18, 300.00, '2025-10-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (19, 25.00, '2025-10-15', 'pas encore effectué', 1, '2025-10-15', '2025-10-31'),  -- Paiement mensuel
    (20, 100.00, '2025-11-01', 'validé', 2, '2025-11-01', '2025-12-31'),  -- Paiement trimestriel
    (21, 300.00, '2025-11-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (22, 25.00, '2025-11-15', 'validé', 1, '2025-11-01', '2025-11-30'),  -- Paiement mensuel
    (23, 100.00, '2025-12-01', 'pas encore effectué', 2, '2025-12-01', '2025-02-28'),  -- Paiement trimestriel non effectué
    (24, 300.00, '2025-12-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel pour un autre utilisateur
    (26, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (27, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (28, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (29, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (30, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (31, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (32, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (33, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (34, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (35, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (36, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (37, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (38, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (39, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (40, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (41, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (42, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (43, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (44, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (45, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (46, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (47, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (48, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (49, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (50, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (51, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (52, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (53, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (54, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (55, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (56, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (57, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (58, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (59, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (60, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (61, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (62, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (63, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (64, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (65, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (66, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (67, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (68, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (69, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (70, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (71, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (72, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (73, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (74, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (75, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (76, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (77, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (78, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (79, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (80, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (81, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (82, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (83, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (84, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (85, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (86, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (87, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (88, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (89, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (90, 25.00, '2025-01-01', 'validé', 1, '2025-01-01', '2025-01-31'),  -- Paiement mensuel
    (91, 100.00, '2025-01-15', 'en attente', 2, '2025-01-01', '2025-03-31'),  -- Paiement trimestriel
    (92, 300.00, '2025-02-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (93, 25.00, '2025-02-01', 'validé', 1, '2025-02-01', '2025-02-28'),  -- Paiement mensuel pour février
    (94, 100.00, '2025-03-01', 'pas encore effectué', 2, '2025-04-01', '2025-06-30'),  -- Paiement trimestriel non effectué
    (95, 300.00, '2025-03-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel pour un autre utilisateur
    (96, 25.00, '2025-03-15', 'validé', 1, '2025-03-01', '2025-03-31'),  -- Paiement mensuel
    (97, 100.00, '2025-04-01', 'en attente', 2, '2025-04-01', '2025-06-30'),  -- Paiement trimestriel
    (98, 300.00, '2025-05-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (99, 25.00, '2025-05-01', 'validé', 1, '2025-05-01', '2025-05-31'),  -- Paiement mensuel
    (100, 100.00, '2025-06-01', 'pas encore effectué', 2, '2025-07-01', '2025-09-30'),  -- Paiement trimestriel non effectué
    (101, 300.00, '2025-07-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (102, 25.00, '2025-07-01', 'validé', 1, '2025-07-01', '2025-07-31'),  -- Paiement mensuel
    (103, 100.00, '2025-08-01', 'en attente', 2, '2025-08-01', '2025-10-31'),  -- Paiement trimestriel
    (104, 300.00, '2025-09-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (105, 25.00, '2025-09-01', 'validé', 1, '2025-09-01', '2025-09-30'),  -- Paiement mensuel
    (106, 100.00, '2025-10-01', 'en attente', 2, '2025-10-01', '2025-12-31'),  -- Paiement trimestriel
    (107, 300.00, '2025-10-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (108, 25.00, '2025-10-15', 'pas encore effectué', 1, '2025-10-15', '2025-10-31'),  -- Paiement mensuel
    (109, 100.00, '2025-11-01', 'validé', 2, '2025-11-01', '2025-12-31'),  -- Paiement trimestriel
    (110, 300.00, '2025-11-01', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel
    (111, 25.00, '2025-11-15', 'validé', 1, '2025-11-01', '2025-11-30'),  -- Paiement mensuel
    (112, 100.00, '2025-12-01', 'pas encore effectué', 2, '2025-12-01', '2025-02-28'),  -- Paiement trimestriel non effectué
    (113, 300.00, '2025-12-10', 'validé', 3, '2025-01-01', '2025-12-31'),  -- Paiement annuel pour un autre utilisateur
    (114, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (115, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (116, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (117, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (118, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (119, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (120, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (121, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (122, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (123, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (124, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (125, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (126, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (127, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (128, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (129, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (130, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (131, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (132, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (133, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (134, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (135, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (136, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (137, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (138, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (139, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (140, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (141, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (142, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (143, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (144, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (145, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (146, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (147, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (148, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (149, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31'),
    (150, 25.00, '2025-12-01', 'validé', 1, '2025-12-01', '2025-12-31');



INSERT INTO categories (nom) VALUES ('short');
INSERT INTO categories (nom) VALUES ('t-shirt');
INSERT INTO categories (nom) VALUES ('rashguard');
INSERT INTO categories (nom) VALUES ('gi');
INSERT INTO categories (nom) VALUES ('sac de transport');
INSERT INTO categories (nom) VALUES ('sweat');

INSERT INTO tailles (nom) VALUES 
('S'), 
('M'), 
('L'), 
('XL');
...
('Rashguard manches longues noir', 'Rashguard respirant et compressif pour l\'entraînement de grappling.', 34.99, 'https://example.com/rashguard-noir.jpg', 1),
('Short de grappling Venum', 'Short sans poches pour une pratique sécurisée du JJB.', 29.99, 'https://example.com/short-venum.jpg', 2),
('Kimono BJJ blanc - A2', 'Kimono de jiu-jitsu brésilien, coupe compétiteur.', 89.99, 'https://example.com/kimono-blanc.jpg', 3),
('Ceinture bleue BJJ', 'Ceinture officielle pour le jiu-jitsu brésilien, avec bande noire.', 14.99, 'https://example.com/ceinture-bleue.jpg', 4),
('Sac de sport JJB', 'Sac à dos résistant pour transporter ton équipement.', 39.99, 'https://example.com/sac-jjb.jpg', 5);
...
INSERT INTO types_messages_personnalises (title, content, created_at, updated_at) VALUES
('Bienvenue', 'Bienvenue sur notre plateforme !', NOW(), NOW()),
('Rappel entraînement', 'N\'oubliez pas l\'entraînement de ce soir à 19h.', NOW(), NOW()),
('Fermeture exceptionnelle', 'Le club sera fermé ce vendredi en raison d\'un événement.', NOW(), NOW()),
('Infos compétition', 'La compétition régionale aura lieu le 10 juin à Bruxelles.', NOW(), NOW()),
('Mise à jour profil', 'Merci de mettre à jour vos informations dans votre espace membre.', NOW(), NOW()),
('Rappel 1 - Facture impayée', 
 'Bonjour, nous vous rappelons que votre facture reste impayée. Merci de régulariser votre situation dans les plus brefs délais.', 
 NOW(), NOW()),
('Rappel 2 - Facture toujours en attente', 
 'Bonjour, sauf erreur de notre part, votre facture est toujours en attente de paiement. Merci de procéder au règlement au plus vite.', 
 NOW(), NOW()),
('Dernier rappel - Suspension possible', 
 'Bonjour, malgré nos rappels précédents, votre facture reste impayée. Sans régularisation dans les 5 jours, nous serons contraints de suspendre votre accès.', 
 NOW(), NOW()),
('Confirmation de paiement reçu', 
 'Bonjour, nous avons bien reçu votre paiement. Merci et à bientôt !', 
 NOW(), NOW());
...
-- Insère chaque utilisateurs à chaques cours --
INSERT INTO inscriptions (utilisateur_id, cours_id, date_inscription, status_id)
SELECT u.id, c.id, CURDATE(), NULL
FROM utilisateurs u
CROSS JOIN cours c
WHERE NOT EXISTS (
    SELECT 1
    FROM inscriptions i
    WHERE i.utilisateur_id = u.id AND i.cours_id = c.id
);